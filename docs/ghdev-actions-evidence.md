...
3. for the dedicated Docker runner container, use `--security-opt systempaths=unconfined` so Docker does not mask/read-only-submount parent `/proc` paths that make the nested fresh procfs fail with `VFS: Mount too revealing`; verify the effective container has `MaskedPaths=[]` and `ReadonlyPaths=[]` while retaining non-root UID, `cap-drop=ALL`, `no-new-privileges`, custom seccomp, read-only rootfs, no host bind mounts, and no Docker socket. The Docker CLI handles `systempaths=unconfined` client-side and removes that token before the daemon request, so readiness proves its effective persisted state through the two empty path arrays rather than requiring the literal option in `HostConfig.SecurityOpt`;
...
5. run the dedicated runner container with exact CPU, memory, PID/process-count, named-volume, tmpfs, healthcheck, and network limits recorded in `/etc/ghdev/runner-readiness.json`; every allowed persistent volume must be an ordinary Docker `local`/`local` named volume with no driver `Options` (local-driver bind backing is forbidden), and the host seccomp file must already contain its final frozen bytes **before** the container is created. Docker exposes different persisted evidence surfaces for different mount mechanisms. A named volume supplied with `-v name:/destination` may appear in `HostConfig.Binds`, while top-level `Mounts` correctly classifies it as `Type=volume`; readiness therefore proves the no-host-bind invariant from top-level `Mounts`, requiring each configured named volume exactly once with exact name/destination/boolean `RW`, rejecting every `Type=bind` or other unsupported top-level mount type, and independently requiring `local`/`local`/no-options volume provenance. The documented temporary filesystem is supplied with Docker `--tmpfs`, whose persisted contract is `HostConfig.Tmpfs`; that form is not required to appear as top-level `Mounts.Type=tmpfs`. Readiness requires `HostConfig.Tmpfs` to match the configured destination/options map exactly and each configured options string to contain exactly one explicit `rw` or `ro` token. Alternate `--mount type=tmpfs`-style top-level evidence is not equivalent for this schema and fails closed rather than being partially admitted without the exact `HostConfig.Tmpfs` option contract. A file-backed `--security-opt seccomp=/path/profile.json` is likewise a Docker CLI input: the CLI reads/compacts the JSON and the daemon persists an inline `seccomp=<JSON>` option, so readiness binds the frozen host path/hash/time and separately requires the daemon-applied inline document to be semantically identical, including lossless comparison of JSON numeric literals such as 64-bit seccomp argument values;
...
`/etc/ghdev/runner-readiness.json` is host-owned, contains no credentials, and uses schema `ghdev-runner-readiness-v1`. It binds the repository, container name, exact Docker image ID, exact GitHub routing labels, named/bridge (never host/none/other-container-shared) Docker network, exact healthcheck vector, frozen seccomp path/hash, CPU/memory/PID limits, allowed named volumes/tmpfs, exact non-secret `.runner` settings path, and exact Runner.Listener path/version/SHA-256. The readiness CLI independently reads the current seccomp file plus nanosecond mtime/ctime, `docker inspect` output and container creation time, each named volume's Docker driver/scope/options, the container's non-secret `.runner` settings, `Runner.Listener --version` and SHA-256, and the authenticated GitHub repository runners API. It fails closed unless all of the following remain true: persisted `DisableUpdate=true`, non-ephemeral exact-repository registration; `.runner` AgentId/AgentName uniquely match a GitHub-side runner that is `online`, idle, and has exactly `self-hosted,Linux,X64,ghdev-verify`; exact runner version/binary; explicit non-root UID (including rejection of alternate numeric spellings of UID 0); `Privileged=false`; read-only rootfs; `CapDrop=ALL` and no added capabilities; exactly one normalized `no-new-privileges` option; exactly one daemon-persisted custom inline seccomp JSON document semantically identical to the frozen host profile under lossless JSON-number comparison, whose current SHA-256 matches configuration and whose mtime and ctime both predate container creation; explicitly present empty-array Docker `MaskedPaths` and `ReadonlyPaths` as the effective persisted proof of the required `systempaths=unconfined` bootstrap posture; no unexpected additional security options; no host namespace/device mounts; top-level Docker `Mounts` containing each exact configured named volume once with exact name/destination/boolean `RW`, with duplicate/missing volumes, every `Type=bind`, top-level `Type=tmpfs`, or other unsupported mount type rejected regardless of `HostConfig.Binds` creation metadata; ordinary local named volumes with no driver options or bind backing; the exact documented `--tmpfs` destination/options map persisted in `HostConfig.Tmpfs`, with each configured options string containing exactly one explicit `rw` or `ro` token; exact resource set; `unless-stopped`; exact persistent-listener/update-disabled Docker labels; no credential-like static environment variables; exact healthcheck; and running/healthy state. When tmpfs entries are configured, missing, null, malformed, extra, or option-mismatched `HostConfig.Tmpfs` evidence fails closed. Top-level `Mounts` is intentionally not treated as an exhaustive census of Docker `--tmpfs` mounts. Missing or malformed named-volume mount evidence and missing or `null` system-path fields are likewise incomplete evidence and fail closed. The readiness command therefore requires local authenticated `gh` read access in addition to Docker inspection; scheduler readiness is not inferred from container metadata alone.
