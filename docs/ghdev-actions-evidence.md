...
5. run the dedicated runner container with exact CPU, memory, PID/process-count, named-volume, tmpfs, healthcheck, and network limits recorded in `/etc/ghdev/runner-readiness.json`; every allowed persistent volume must be an ordinary Docker `local`/`local` named volume with no driver `Options` (local-driver bind backing is forbidden), and the host seccomp file must already contain its final frozen bytes **before** the container is created. Docker creation-request metadata is not effective-mount authority: a named volume supplied with `-v name:/destination` may appear in `HostConfig.Binds`, while top-level `Mounts` correctly classifies it as `Type=volume`. Readiness therefore proves the no-host-bind invariant from an exhaustive top-level `Mounts` census that admits each configured `Type=volume` and `Type=tmpfs` destination exactly once, requires structurally valid boolean `RW` evidence, requires each tmpfs configuration to contain exactly one explicit `rw` or `ro` token and the effective tmpfs `RW` value to match it, and rejects duplicates, missing destinations, every `Type=bind`, or any other unexpected mount type, together with independent `local`/`local`/no-options volume provenance. A file-backed `--security-opt seccomp=/path/profile.json` is likewise a Docker CLI input: the CLI reads/compacts the JSON and the daemon persists an inline `seccomp=<JSON>` option, so readiness binds the frozen host path/hash/time and separately requires the daemon-applied inline document to be semantically identical, including lossless comparison of JSON numeric literals such as 64-bit seccomp argument values;
