...
`restore` is receipt-bound rather than a generic mutation capability. The guard admits only the exact receipt path/digest previously authenticated when that operation reached the `quarantined` ledger phase; a caller-supplied replacement digest cannot mint restore authority. Before writing, restore requires absent destinations, intact quarantine inventory, no current-index tracking of the requested paths, and the exact post-quarantine dirty-state fingerprint. It never overwrites later content. Restored path type, mode, bytes, and symlink target must reproduce the receipt inventory and original Git dirty-state/path fingerprints. The receipt and quarantine are preserved on failure. Restore can continue after a fresh plugin process because the operation ledger is persisted, but it cannot release target authority, satisfy HEAD proof, advance Fresh-review/Verify generations, or mint target-lifecycle state, and it is blocked while authenticated `OWNER_RECONCILIATION` is active.
