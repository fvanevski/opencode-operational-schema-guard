...
Normalize a non-empty bounded Task prompt into the Turn-1 envelope when safe. Explore and Verify retain the existing short envelope-less inference boundary. Fresh-review may infer a fully envelope-less prompt beyond that legacy short-prompt cliff only when the deterministic wrapper plus role contract fits the existing Fresh-review prompt limit, the non-empty description is itself a deterministic bounded summary suitable for the derived Scope and names the same boundary class as the proven directive (finite targets/files/paths versus diff/patch/change-set), and the original prompt supplies deterministic closed-scope syntax. The first non-empty directive must itself close review scope with only/exactly/strictly or a limit/restrict/confine-to form directly onto either a finite in-limit filesystem target list or a current/provided/attached diff, patch, PR, change-set, or changed-file anchor; a bare path/diff mention or a generic "review only" phrase elsewhere before a path is not sufficient. That closed directive may use only the narrow admitted result suffixes, and no additional envelope-less prose is inferred after it. Any richer context or additional instruction must use the explicit packet envelope. Complex or ambiguous review instructions must use the explicit packet envelope. Preserve the original prompt verbatim as supporting context. Repository-wide or open-ended scope language vetoes Fresh-review inference, as do generic traversal/inspection beyond the finite anchor, explicit review-scope widening or beyond-anchor instructions, investigative anything/everything/whatever scope, and contradictory rest/surrounding/adjacent/broader-scope language; the inferred Question and Stop condition never manufacture boundedness. Partial or ambiguous envelopes, unsafe scope, target overflow, and oversized or semantically unbounded delegation remain fail-closed. A child capability mismatch rejects only that invocation and provides a machine-readable correction code plus the supported route; it never terminalizes the child.
...
Semantic-review ownership is an explicit typed workflow decision, never inferred from free-form prose or consequence category. The primary declaration SEMANTIC REVIEW AUTHORITY: central-owned|local-fresh-review|both controls only local semantic-review routing: central-owned elides Fresh-review, records local semantic review as NOT_EVALUATED, and preserves the separate Central semantic-review requirement; local-fresh-review and both preserve the strict Fresh-review result contract. Repository-final Verify authority is likewise explicit: REPOSITORY VERIFY AUTHORITY: local-verify is the default, while trusted-actions removes only the pre-publication local repository-final Verify gate and still requires a fresh post-publication exact-head trusted Actions PASS before acceptance. Reuse of an already-materialized Actions receipt additionally requires a primary TRUSTED ACTIONS RECEIPT declaration binding the exact head and immutable receipt SHA-256 plus one Evidence bundle under /tmp/opencode/verify/materials; an old receipt cannot certify active uncommitted edits. For broad child work, prefer the deterministic plan-child-work.mjs typed planner before Task launch; it derives live diff/hunk complexity from an exact declared/campaign base when available, emits bounded complete partitions or UNREPRESENTABLE without silently dropping targets, preserves the normalized packet's non-target supporting context when rendering live canonical partitions, admits only the exact normalized canonical partition packets it previously returned, and advances a partitioned Fresh-review/Verify generation only after every packet in the generation-bound aggregate obligation succeeds. While such a Fresh-review or Verify obligation remains outstanding, alternate same-role READY packets, replans, and trusted-receipt elision are rejected; only the obligation's exact canonical packets may discharge that gate until a new edit or authority epoch invalidates it. The aggregate obligation and completed canonical membership are workspace-persisted across session and plugin-process restart. A current equivalent trusted self-hosted Actions receipt satisfies repository-final deterministic Verify without launching a duplicate local Verify child; it never satisfies semantic review or genuinely host-specific evidence.
...
When pending or mismatched exact-head authority is blocked by Git-visible untracked material, preserve it only through /home/filip/.config/opencode/plugins/operational-schema-v5/scripts/untracked-quarantine.mjs --spec /tmp/opencode/verify/untracked-quarantine/specs/<file>.json for the same governed workspace and exact authority binding. Use inspect first; quarantine must bind the authenticated returned workspace/status/path/inventory SHA-256 values, and restore must consume the exact authenticated prior receipt. This is preservation/recovery only: generic rm, mv, cp, git clean, git reset, checkout/reset cleanup, stash, or equivalent workspace mutation remains governed and cannot substitute for the typed helper. The helper never proves HEAD, releases target authority, or satisfies review/Verify.
When a deterministic authority or lifecycle block rejects a command, do not ask for a conversational override that cannot change persisted authority; use the stated typed capability if admitted, otherwise report the blocker. Persisted target authority is never released merely because the session changes or work is idle. When the user explicitly changes the task to a different starting revision, a new primary declaration `REQUIRED STARTING HEAD: <40-hex>` or strict-start alias `REQUIRED STARTING HEAD SHA: <40-hex>` is the contract-defined authority transition; after a bare `git rev-parse HEAD` proof it supersedes incompatible prior-target state and prior-head review/Verify/delegation coverage. Establishing a new binding after a recognized terminal target release is likewise a new authority epoch; prior review/Verify/delegation state cannot cross the unbound interval. Repeating the currently bound target SHA in a strict-start declaration is not a revision transition and must not change target mode or clear authenticated OWNER_RECONCILIATION state. Exact local-agent-assessment.mjs and reconcile-owner-base.mjs wrappers remain one logical bare invocation: they may be formatted on one physical line or with unquoted backslash-LF/CRLF continuations only when those continuations canonicalize to the exact admitted argv; unescaped multiline input, operators, redirects, substitutions, extra args, wildcards, and malformed SHAs remain rejected. Keep host-evidence extraction bounded: prefer selectors and concise summaries over dumping large JSON, JUnit, or other structured payloads into primary context, especially after an initialization-derived context-pressure warning. Never generate or execute ad-hoc /tmp/*.sh orchestration; use bare commands or mandated deterministic helpers.
...
## Hybrid workflow planning and evidence reuse

Issue #15 adds deterministic routing primitives around the existing guard rather than weakening its safety boundaries. The workflow separates **Source** evidence (exact repository/GitHub/runner facts) from **Constraint** policy (which evidence classes and semantic-review authorities are required). `central-owned`, `local-fresh-review`, and `both` are explicit semantic-review modes; the guard never infers them from prose. `central-owned` launches no local Fresh-review child and reports local semantic review as `NOT_EVALUATED`, while Central semantic review remains mandatory.

For work that may exceed a child packet's bounded limits, stage an `opencode-child-work-plan-v1` JSON document under `/tmp/opencode/verify/materials/` and run:
...
The planner uses target count, file size, diff size/hunk complexity, and production-vs-test/document role to produce stable `READY` or `PARTITION_REQUIRED` packets with complete unique target coverage. If one target cannot be represented safely, it returns `UNREPRESENTABLE`; it never truncates or silently drops scope. Live Task preflight invokes the same planner when exact authority and explicit targets are available. When an exact `EXPECTED_BASE_SHA`, `REVIEW_BASE_SHA`, or `BASE_SHA` is declared (or the edit campaign has an exact base), live planning obtains each target's patch bytes and hunk count from a read-only exact-base Git diff; unavailable diff evidence fails conservatively rather than being treated as zero complexity. Partition-required preflight preserves the already-normalized packet verbatim except for replacing its `Targets:` bullets with each exact subset, so bounded supporting context, target annotations, and injected child contracts survive every canonical packet. Those packets form one generation-bound aggregate obligation: Fresh-review/Verify publication generations advance only after every required partition succeeds, duplicate completed partitions are rejected while the aggregate is outstanding, alternate same-role READY/replan/receipt-elision routes cannot bypass it, and any subsequent implementation edit or authority epoch invalidates the obligation. Aggregate obligation state and completed canonical membership are workspace-persisted across session and plugin-process restart, so restart does not create an alternate path around the gate. The small-Explore exact-lookup exception is bypassed only for an exact normalized packet hash previously minted by that planner result.
...
Repository-final deterministic Verify uses the trusted self-hosted Actions evidence class when a current equivalent Slice-K receipt exists. The supported workstation deployment keeps its hardened repository-scoped `ghdev-verify` runner container continuously connected with Docker `unless-stopped`, so Central workflow dispatch does not require per-run local activation or token injection; candidate execution remains run-unique, read-only, network-unshared Bubblewrap work. `REPOSITORY VERIFY AUTHORITY: local-verify` is the default. Explicit `REPOSITORY VERIFY AUTHORITY: trusted-actions` permits publication without a duplicate pre-publication local repository-final child but leaves Verify unsatisfied until the freshly published exact head receives trusted Actions PASS. For reuse of an already-materialized receipt, primary authority must declare `TRUSTED ACTIONS RECEIPT: head=<40-lowercase-sha>; sha256=<64-lowercase-sha256>` and the Verify packet must name exactly one `Evidence: /tmp/opencode/verify/materials/<bundle>.json`; the bundle uses `ghdev-actions-plan-evidence-v1` and contains the receipt plus the trusted profile used to validate it. The guard rejects digest/head mismatch and refuses to use a remote receipt to certify an active uncommitted edit generation. Evidence reuse is keyed by exact candidate head and generation/scope plus profile, dependency/config, environment, and stable trusted producer/controller fingerprints. The immutable workflow run/artifact identity and command/test counts remain attached to each receipt, but run-specific artifact IDs are not themselves producer-equivalence keys. A mismatched head or dependent provenance component invalidates reuse; an Actions receipt never substitutes for semantic review or host/runtime/service-specific evidence. Child result parsing accepts benign semicolon/multiline/field-order/whitespace variation but still rejects duplicate/conflicting required fields, missing fields, incidental narrative markers, and incomplete success counts. Gateway-owned detached assessment may preserve a pre-existing dirty owner checkout only by proving an exact Git-visible dirty-state content fingerprint before and after; the dirty-path census remains byte-oriented from Git's NUL-delimited output and records raw path identity without lossy UTF-8 decoding, including filenames that are not valid UTF-8. Repository-owned/base-authority assessment remains strictly clean-owner-only.

## Pending-authority untracked quarantine

Pending or mismatched exact-head authority never authorizes generic cleanup. `rm`, `mv`, `cp`, `git clean`, `git reset`, checkout-based cleanup, stash/reset workflows, and other discard-oriented workspace mutations remain subject to the ordinary fail-closed authority guard. The only agent-side preservation remedy for Git-visible untracked blockers is the typed reversible quarantine helper:

```text
/home/filip/.config/opencode/plugins/operational-schema-v5/scripts/untracked-quarantine.mjs --spec /tmp/opencode/verify/untracked-quarantine/specs/<name>.json
```

The public command is admitted only in its exact bare shape. `inspect` and `quarantine` are admitted only while the exact persisted `target` or `strict-start` authority is `pending` or `mismatch`; verified authority does not admit preservation. Specs bind the governed workspace, authority mode/binding, a unique operation ID, a finite canonical repository-relative path set, and byte-oriented Git dirty-state/path/inventory fingerprints. The spec must be one direct concrete JSON file beneath the harness-owned spec root. The wrapper independently performs the `inspect` and persists an authenticated operation ledger across sessions/processes before a later `quarantine` can be admitted; operation IDs cannot be silently reused. The helper independently reruns NUL-delimited Git status plus index checks before mutation; every requested path must be represented only by `??` records and must contain no tracked, staged, conflicted, submodule, ignored-only, escaping/overlapping, non-UTF-8, or unsupported special-file state. Non-UTF-8 path bytes are rejected fail-closed rather than decoded lossily.

Quarantine first inventories and hashes supported files/directories/symlinks, copies them beneath `/tmp/opencode/verify/untracked-quarantine/operations/<operation-id>/data`, verifies the complete copy, and writes a read-only SHA-256-bound recovery receipt **before** any original is removed. The receipt binds the workspace identity, authority, exact paths, pre-operation dirty-state/path fingerprints, precomputed expected post-quarantine status, inventory/hash, and quarantine location. Re-entry with the same exact operation may safely finish an interrupted removal only when every remaining source object is still an exact subset of the receipt inventory. Post-quarantine readback must prove the intended paths are absent and tracked/staged state did not change.

`restore` is receipt-bound rather than a generic mutation capability. The guard admits only the exact receipt path/digest previously authenticated when that operation reached the `quarantined` ledger phase; a caller-supplied replacement digest cannot mint restore authority. Before writing, restore requires absent destinations, intact quarantine inventory, and no current-index tracking of the requested paths. Intervening unrelated workspace or HEAD state may change during the guarded operation; restore preserves the current tracked/staged state while reproducing only the receipt-bound requested paths. It never overwrites later content. Restored path type, mode, bytes, and symlink target must reproduce the receipt inventory and original Git dirty-state/path fingerprints. The receipt and quarantine are preserved on failure. Restore can continue after a fresh plugin process because the operation ledger is persisted, but it cannot release target authority, satisfy HEAD proof, advance Fresh-review/Verify generations, or mint target-lifecycle state, and it is blocked while authenticated `OWNER_RECONCILIATION` is active.

Typical two-stage preservation uses an `inspect` spec first, then copies the returned `workspace_sha256`, `status_sha256`, `paths_sha256`, and `inventory_sha256` into `expected_workspace_sha256`, `expected_status_sha256`, `expected_paths_sha256`, and `expected_inventory_sha256` in a `quarantine` spec for the same operation/workspace/authority/path set. After the guarded operation is complete, an exact `restore` spec supplies the emitted `receipt_path` and `receipt_sha256`; those values must also match the guard's authenticated quarantine ledger. This mechanism preserves user artifacts; it is not permission to discard, rewrite, stash, or semantically remediate them.
...
- safe preflight normalization of bounded Task prompts with the existing short envelope-less inference boundary retained for Explore/Verify; Fresh-review alone may infer beyond the legacy short-prompt cliff when the resulting packet fits its existing role limit, the description is a deterministic bounded summary suitable for the derived Scope and names the same boundary class as the proven directive (finite targets/files/paths versus diff/patch/change-set), and the first non-empty directive syntactically closes scope with only/exactly/strictly or a limit/restrict/confine-to form directly onto a finite target/diff anchor; generic closure wording elsewhere before a path is insufficient, only narrow result suffixes may follow that directive, and no additional envelope-less prose may follow it; richer context or additional instructions require the explicit packet envelope; contradictory rest/surrounding/adjacent/broader scope, repository-wide/open-ended scope, generic traversal/inspection beyond that finite anchor, investigative anything/everything/whatever scope, partial/ambiguous envelopes, and semantic/routing overflows remain fail-closed;
...
- explicit shell-mutation tracking plus the locally required publication gates after multi-file, shell-mutating, or high-risk edits; `central-owned` semantic-review authority removes only local Fresh-review and leaves Central semantic review required, while `local-fresh-review`/`both` preserve Fresh-review; repository-final deterministic Verify may be satisfied by an equivalent trusted Actions receipt rather than a duplicate local child; content-neutral `/tmp` evidence redirection does not fabricate a new workspace edit or invalidate current gates;
