...
`restore` is receipt-bound rather than a generic mutation capability. It requires the exact receipt digest, exact workspace/authority/path identity, intact quarantine inventory, and absent destinations; it never overwrites later content. Restored path type, mode, bytes, and symlink target must reproduce the receipt inventory and original Git dirty-state/path fingerprints. The receipt and quarantine are preserved on failure. Restore cannot release target authority, satisfy HEAD proof, advance Fresh-review/Verify generations, or mint target-lifecycle state, and it is blocked while authenticated `OWNER_RECONCILIATION` is active.

Typical two-stage preservation uses an `inspect` spec first, then copies the returned `workspace_sha256`, `status_sha256`, and `paths_sha256` into a `quarantine` spec for the same operation/workspace/authority/path set. After the guarded operation is complete, an exact `restore` spec supplies the emitted `receipt_path` and `receipt_sha256`. This mechanism preserves user artifacts; it is not permission to discard, rewrite, stash, or semantically remediate them.
