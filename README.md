...
- safe preflight normalization of bounded Task prompts with the existing short envelope-less inference boundary retained for Explore/Verify; Fresh-review alone may infer beyond the legacy short-prompt cliff when the resulting packet fits its existing role limit, the description is a deterministic bounded summary suitable for the derived Scope, and the first non-empty directive syntactically closes scope with only/exactly/strictly or a limit/restrict/confine-to form directly onto a finite target/diff anchor; generic closure wording elsewhere before a path is insufficient, only narrow result suffixes may follow that directive, and no additional envelope-less prose may follow it; richer context or additional instructions require the explicit packet envelope; contradictory rest/surrounding/adjacent/broader scope, repository-wide/open-ended scope, generic traversal/inspection beyond that finite anchor, investigative anything/everything/whatever scope, partial/ambiguous envelopes, and semantic/routing overflows remain fail-closed;
