...
- safe preflight normalization of fully envelope-less bounded Task prompts without an arbitrary short-prompt cliff, preserving the original prompt verbatim as supporting context when the inferred envelope plus role contract still fits the existing role limit; partial/ambiguous envelopes and semantic/routing overflows remain fail-closed;
