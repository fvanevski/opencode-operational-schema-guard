...
- safe preflight normalization of bounded Task prompts with the existing short envelope-less inference boundary retained for Explore/Verify; Fresh-review alone may infer beyond the legacy short-prompt cliff when the resulting packet fits its existing role limit and the original prompt explicitly closes scope with only/exactly/strictly or a limit/restrict/confine-to form around a finite target/diff anchor; a bare path or diff mention is not sufficient, and contradictory rest/surrounding/adjacent/broader scope, repository-wide/open-ended scope, generic traversal/inspection beyond that finite anchor, investigative anything/everything/whatever scope, partial/ambiguous envelopes, and semantic/routing overflows remain fail-closed;
