...
- safe preflight normalization of bounded Task prompts with the existing short envelope-less inference boundary retained for Explore/Verify; Fresh-review alone may infer beyond the legacy short-prompt cliff when the resulting packet fits its existing role limit and the original prompt supplies positive finite/diff-anchored bounded-scope evidence, while repository-wide/open-ended scope and generic traversal/inspection beyond that finite anchor, partial/ambiguous envelopes, and semantic/routing overflows remain fail-closed;
