...
function legacyMetrics(messages) {
  const metrics = {
    ...
    exploreLaunches: 0,
    verifyLaunches: 0,
    freshReviewLaunches: 0,
    plannerSuccesses: 0,
    plannerPartitions: 0,
    plannerPreventedLaunches: 0,
    terminalNormalizations: 0,
    freshReviewElisions: 0,
    capabilityPrevented: 0,
  }
  const patterns = [
  ...
  for (const message of messages) {
    ...
    for (const part of Array.isArray(message?.parts) ? message.parts : []) {
      ...
      const errorText = String(part?.state?.error ?? "")
      scan(part?.state?.error)
      if (tool === "task") {
        const role = String(part?.state?.input?.subagent_type ?? "")
        const launched = Boolean(part?.state?.metadata?.sessionId ?? part?.state?.metadata?.sessionID)
        if (launched && role === "explore") metrics.exploreLaunches += 1
        if (launched && role === "verify") metrics.verifyLaunches += 1
        if (launched && role === "fresh-review") metrics.freshReviewLaunches += 1
        if (/HONOR_SEMANTIC_REVIEW_AUTHORITY|Fresh-review is elided/i.test(errorText)) {
          metrics.freshReviewElisions += 1
          metrics.capabilityPrevented += 1
        }
        if (/deterministic planner returned PARTITION_REQUIRED|deterministic-complexity-partition/i.test(errorText)) {
          metrics.plannerPreventedLaunches += 1
          metrics.capabilityPrevented += 1
        }
      }
      if (tool === "task" && !schema) scan(part?.state?.output)
      if (schema && typeof schema === "object") {
        const planning = schema.planning
        if (planning && typeof planning === "object" && planning.status && planning.status !== "LEGACY_OR_UNBOUND") {
          metrics.plannerSuccesses += 1
          if (Number.isInteger(planning.partitionCount)) metrics.plannerPartitions += planning.partitionCount
        }
        if (Array.isArray(schema.terminalNormalizations)) metrics.terminalNormalizations += schema.terminalNormalizations.length
...
function makeCandidate(category, events, index) {
  ...
  const definitions = {
    role_elision: ["prevented_unnecessary_delegation", "Preserve the explicit semantic-review authority; do not reintroduce a child for an elided role."],
    planner_partition: ["deterministic_partition_boundary", "Use the repository-rendered canonical partition packets rather than hand-splitting or retrying the rejected broad packet."],
