...
export function targetComplexity(target) {
  return targetCost(validateTarget(target, 0))
}
...
function normalizedPlanInput(input) {
  ...
  const evidenceEntries = Array.isArray(input.evidence_entries) ? input.evidence_entries : []
  ...
  if (authority.class === "unbound-workspace") {
    if (Object.keys(authority).some((key) => key !== "class")) throw new Error("unbound-workspace authority accepts only the class field")
    if (role !== "explore" || gateClass !== "focused-development" || input.evidence_requirement !== undefined || evidenceEntries.length > 0) {
      throw new Error("unbound-workspace authority is admitted only for non-evidence focused-development Explore planning")
    }
    normalizedAuthority = { class: "unbound-workspace" }
  } else {
    if (authority.class !== undefined && authority.class !== "exact-head") throw new Error("authority.class must be exact-head or unbound-workspace")
    normalizedAuthority = {
      class: "exact-head",
