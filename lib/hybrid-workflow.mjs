...
export function semanticReviewAuthorityFromMessage(text) {
  ...
  const modes = new Set(matches.map((match) => match[1]))
...
function fitPromptPartitions(groups, base, charLimit) {
  ...
  while (queue.length > 0) {
    ...
    queue.unshift(group.slice(middle), group.slice(0, middle))
  }
  ...
  for (const rawLine of String(text ?? "").split(/\r?\n/)) {
    ...
    for (const segment of segments) {
      ...
      for (const key of keys) {
        ...
        if (existing !== undefined && existing !== value) reasons.push(`conflicting-${canonicalKey.toLowerCase()}`)
...
export function replayFrictionScenario(mode) {
  ...
  return {
    ...
    actions_executions: 1,
    actions_receipt_reuses: 2,
