...
async function readEvidenceSnapshot(path) {
...
async function readRepositoryOwnedEvidence(path, spec, runExit) {
  if (!(await pathExists(path))) throw assessmentError("repository-owned runner did not create its typed evidence file", "INFRA_ERROR")
  ...
  const snapshot = await readEvidenceSnapshot(canonical)
  if (!snapshot) throw assessmentError("repository-owned evidence disappeared before it could be snapshotted", "INFRA_ERROR")
...
export async function runRepoPrAssessment(specInput, {
  ...
  try {
    ...
    summary.runner.run = { exit: runResult.code, signal: runResult.signal }
    const evidence = await readEvidenceSnapshot(evidencePath)
    if (evidence) {
      summary.runner_evidence_sha256 = evidence.sha256
      summary.runner_evidence_bytes = evidence.bytes
    }
