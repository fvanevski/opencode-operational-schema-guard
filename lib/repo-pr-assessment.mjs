...
function runProcess(executable, argv, cwd, environment, {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.once("close", (rawCode, processSignal) => {
      ...
      if (status?.kind === "runner" && Number.isInteger(status.code) && status.code >= 0 && status.code <= 255) {
      ...
      if (status?.kind === "control-mutation") {
      ...
      if (status?.kind === "setup-error") {
        resolvePromise({ code: 1, signal: processSignal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: false })
        return
      }
      if (status?.kind === "reap-error") {
        resolvePromise({ code: 1, signal: processSignal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: true })
        return
      }
      resolvePromise({ code: rawCode ?? 1, signal: processSignal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: true })
    })
