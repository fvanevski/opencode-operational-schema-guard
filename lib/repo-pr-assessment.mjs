...
async function readEvidenceSnapshot(anchor, relativePath) {
  ...
  for (const part of parts.slice(0, -1)) {
    ...
    try {
      ...
      throw assessmentError(`runner evidence parent cannot be inspected (${error.code ?? error.message})`, "INFRA_ERROR")
    }
    if (info.isSymbolicLink() || !info.isDirectory()) throw assessmentError("runner evidence parent must be a real directory", "INFRA_ERROR")
  }
