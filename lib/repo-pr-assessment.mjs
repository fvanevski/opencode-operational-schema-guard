...
export function parseRepoPrAssessmentSpec(value) {
  ...
  const headRef = safeRefName(repository.head_ref, "repository.head_ref")
  ...
  const planArgv = runner.plan_argv === undefined ? undefined : validateTemplateArgv(runner.plan_argv, "runner.plan_argv", { required: requiredAuthority })
  if (planArgv?.some((arg) => arg.includes("{evidence_path}"))) throw assessmentError("runner.plan_argv must not bind {evidence_path}")
  const runArgv = validateTemplateArgv(runner.run_argv, "runner.run_argv", { required: [...requiredAuthority, "evidence_path"] })
  ...
  const integrityFiles = value.integrity_files ?? []
  if (!Array.isArray(integrityFiles) || integrityFiles.length > MAX_INTEGRITY_FILES) throw assessmentError(`integrity_files must contain at most ${MAX_INTEGRITY_FILES} paths`)
  const normalizedIntegrity = [...new Set(integrityFiles.map((path) => safeRelativePath(path, "integrity_files entry")))]
  ...
  return {
    ...
    environment: { venv },
    runner: { path: runnerPath, planArgv, runArgv },
