...
async function hashOwnerPath(repoRoot, relativePath) {
  const requested = resolve(repoRoot, relativePath)
  if (!inside(requested, repoRoot)) throw assessmentError(`owner preservation path escapes repository (${relativePath})`, "INFRA_ERROR")
  let info
  try {
    info = await lstat(requested)
  } catch (error) {
    if (error?.code === "ENOENT") return { path: relativePath, kind: "missing" }
    throw assessmentError(`owner preservation path is unreadable (${relativePath}; ${error.code ?? error.message})`, "INFRA_ERROR")
  }
  if (info.isSymbolicLink()) {
    const target = await readlink(requested).catch((error) => {
      throw assessmentError(`owner preservation symlink cannot be read (${relativePath}; ${error.code ?? error.message})`, "INFRA_ERROR")
    })
    return { path: relativePath, kind: "symlink", mode: info.mode, size: info.size, sha256: sha256(Buffer.from(target)) }
  }
  if (!info.isFile()) {
    if (info.isDirectory()) throw assessmentError(`owner preservation cannot prove exact bytes for dirty directory entry (${relativePath})`, "BLOCKED")
    return { path: relativePath, kind: "other", mode: info.mode, size: info.size }
  }
  const handle = await open(requested, constants.O_RDONLY | constants.O_NOFOLLOW).catch((error) => {
    throw assessmentError(`owner preservation file cannot be opened safely (${relativePath}; ${error.code ?? error.message})`, "INFRA_ERROR")
  })
  try {
    ...
    return { path: relativePath, kind: "file", mode: info.mode, size: info.size, sha256: hash.digest("hex") }
  } finally {
    await handle.close()
  }
}
...
async function ownerSnapshot(repoRoot) {
  ...
  const rawStatus = gitRaw(repoRoot, ["status", "--porcelain=v1", "-z", "--untracked-files=all"]).stdout
  const trackedWorking = nulPaths(gitRaw(repoRoot, ["diff", "--name-only", "-z", "--no-ext-diff", "--no-renames"]).stdout)
  const trackedIndex = nulPaths(gitRaw(repoRoot, ["diff", "--cached", "--name-only", "-z", "--no-ext-diff", "--no-renames"]).stdout)
  const untracked = nulPaths(gitRaw(repoRoot, ["ls-files", "--others", "--exclude-standard", "-z"]).stdout)
  const paths = [...new Set([...trackedWorking, ...trackedIndex, ...untracked])].sort()
  const pathState = []
  for (const path of paths) pathState.push(await hashOwnerPath(repoRoot, path))
  const indexDiff = gitRaw(repoRoot, ["diff", "--cached", "--raw", "-z", "--no-ext-diff", "--no-renames"]).stdout
  const fingerprint = sha256(Buffer.from(JSON.stringify({
    head,
    branch,
    raw_status_sha256: sha256(Buffer.from(rawStatus)),
    index_diff_sha256: sha256(Buffer.from(indexDiff)),
    paths: pathState,
  })))
