...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  try {
    ...
    try {
      await mkdir(controlWorktree, { mode: 0o700 })
    } catch (error) {
    ...
    controlAnchor = await anchorDirectory(controlWorktree, "repository-owned control snapshot root")
    ...
    values.repo_root = `/proc/${process.pid}/fd/${controlAnchor.handle.fd}`
