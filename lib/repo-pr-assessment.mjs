...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  try {
    ...
    if (controlWorktreeCreated && !controlWorktreeAdmitted) {
      try {
        await controlGitAnchor?.handle.close().catch(() => {})
        controlGitAnchor = undefined
        await revalidateDirectoryAnchor(controlAnchor, "repository-owned partial control snapshot root")
        await rm(controlWorktree, { recursive: true, force: false })
        await controlAnchor.handle.close().catch(() => {})
        controlAnchor = undefined
        summary.control_snapshot_cleanup = (await pathExists(controlWorktree)) ? "FAILED" : "PASS"
        if (summary.control_snapshot_cleanup !== "PASS") {
          summary.host_evidence_result = "ISOLATION_BREACH"
          summary.error = "repo-pr-assessment: partial repository-owned control snapshot cleanup left the reserved path present"
        }
      } catch (error) {
