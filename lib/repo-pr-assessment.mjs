...
import { mkdir, lstat, open, readFile, readdir, realpath, rm, rmdir, stat, unlink, writeFile } from "node:fs/promises"
...
async function hashOwnerPath(repoRoot, relativePath) {
  ...
  if (info.isSymbolicLink()) {
    const target = await readFile(requested).catch(() => undefined)
    return { path: relativePath, kind: "symlink", mode: info.mode, size: info.size, sha256: target ? sha256(target) : "UNREADABLE" }
  }
