...
export async function runRepoPrAssessment(specInput, {
  ...
  try {
    ...
    for (const integrityEntry of spec.integrityFiles) {
      const entry = await trackedRegularFile(worktree, integrityEntry.path, `integrity file ${integrityEntry.path}`)
      if (integrityEntry.expectedSha256 && integrityEntry.expectedSha256 !== entry.sha256) {
        throw assessmentError(`integrity file ${integrityEntry.path} sha256 does not match the pinned authority`, "BLOCKED")
      }
