  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.once("exit", (rawCode, signal) => {
      ...
      if (supervised && code === SUPERVISOR_DESCENDANTS) {
        resolvePromise({ code: 0, signal: signal ?? null, survivingDescendants: true, controlMutation: false, supervisorError: false, containmentUncertain: false })
        return
      }
      if (supervised && code === SUPERVISOR_CONTROL_MUTATION) {
        resolvePromise({ code: 0, signal: signal ?? null, survivingDescendants: false, controlMutation: true, supervisorError: false, containmentUncertain: false })
        return
      }
      if (supervised && code === SUPERVISOR_SETUP_ERROR) {
        resolvePromise({ code: 1, signal: signal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: false })
        return
      }
      if (supervised && code === SUPERVISOR_REAP_ERROR) {
        resolvePromise({ code: 1, signal: signal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: true })
        return
      }
      resolvePromise({ code, signal: signal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: false, containmentUncertain: false })
    })
  })
}
...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  try {
    ...
    values.repo_root = "/proc/self/cwd"
