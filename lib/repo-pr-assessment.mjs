...
function normalizeIntegrityFiles(value, { requireSha256 = false } = {}) {
  ...
  const entries = value.map((entry) => {
    if (typeof entry === "string") {
      if (requireSha256) throw assessmentError("repository-owned integrity_files entries must pin sha256")
      return { path: safeRelativePath(entry, "integrity_files entry"), expectedSha256: undefined }
    }
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      throw assessmentError("integrity_files entries must be paths or {path, sha256} objects")
    }
    const path = safeRelativePath(entry.path, "integrity_files entry.path")
    if (!SHA256.test(entry.sha256 ?? "")) throw assessmentError(`integrity_files sha256 is invalid for ${path}`)
    return { path, expectedSha256: entry.sha256 }
  })
  if (new Set(entries.map((entry) => entry.path)).size !== entries.length) {
    throw assessmentError("integrity_files must not contain duplicate paths")
  }
  if (requireSha256 && entries.length === 0) {
...
export function parseRepoPrAssessmentSpec(value) {
  ...
  if (execution === "gateway-owned") {
    if (authority !== undefined || runnerSha256 !== undefined || resultContract !== undefined) {
    ...
    if (!SHA256.test(runnerSha256 ?? "")) throw assessmentError("repository-owned runner.sha256 must be 64 lowercase hexadecimal characters")
  ...
  const normalizedIntegrity = normalizeIntegrityFiles(value.integrity_files ?? [], { requireSha256: execution === "repository-owned" })
  ...
  return {
    ...
    environment: { venv },
    runner: { path: runnerPath, execution, authority, sha256: runnerSha256, resultContract, planArgv, runArgv },
...
async function maybeHashEvidence(path) {
  if (!(await pathExists(path))) return undefined
  const info = await lstat(path)
  if (info.isSymbolicLink() || !info.isFile()) throw assessmentError("runner evidence must be a regular non-symlink file", "INFRA_ERROR")
...
async function pinnedRepositoryFile(repoRoot, authoritySha, relativePath, expectedSha256, label) {
  const file = await trackedRegularFile(repoRoot, relativePath, label)
  if (!SHA256.test(expectedSha256 ?? "") || file.sha256 !== expectedSha256) {
  ...
  if (workingBlob !== parts[2]) throw assessmentError(`${label} working bytes differ from the authority SHA`, "STALE")
  return file
}
...
async function readRepositoryOwnedEvidence(path, spec, runExit) {
  const hashed = await maybeHashEvidence(path)
  if (!hashed) throw assessmentError("repository-owned runner did not create its typed evidence file", "INFRA_ERROR")
  const root = await realpath(ASSESSMENT_NATIVE_RESULT_ROOT).catch((error) => {
    throw assessmentError(`repository-owned evidence root is unavailable (${error.code ?? error.message})`, "INFRA_ERROR")
  })
  const canonical = await realpath(path).catch((error) => {
    throw assessmentError(`repository-owned evidence cannot be resolved (${error.code ?? error.message})`, "INFRA_ERROR")
  })
  if (!inside(canonical, root)) throw assessmentError("repository-owned evidence escaped the bounded result root", "INFRA_ERROR")
  let document
  try {
    document = JSON.parse((await readFile(canonical)).toString("utf8"))
  } catch (error) {
  ...
  return { ...hashed, document, cleanupPass }
}
...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  const evidencePath = nativeEvidencePath(spec)
  ...
  try {
    ...
    const runner = await pinnedRepositoryFile(repoRoot, authoritySha, spec.runner.path, spec.runner.sha256, "runner.path")
    ...
    for (const entry of spec.integrityFiles) {
      const pinned = await pinnedRepositoryFile(repoRoot, authoritySha, entry.path, entry.expectedSha256, `integrity file ${entry.path}`)
    ...
    const evidence = await readRepositoryOwnedEvidence(evidencePath, spec, runResult.code)
...
export async function runRepoPrAssessment(specInput, {
  ...
  try {
    ...
    const refspecs = [...new Set([
      `+refs/heads/${spec.repository.baseRef}:refs/remotes/${spec.repository.remote}/${spec.repository.baseRef}`,
      `+refs/heads/${spec.repository.headRef}:refs/remotes/${spec.repository.remote}/${spec.repository.headRef}`,
    ])]
    git(repoRoot, ["fetch", "--no-tags", spec.repository.remote, ...refspecs])
    const observedBase = git(repoRoot, ["rev-parse", "--verify", `refs/remotes/${spec.repository.remote}/${spec.repository.baseRef}^{commit}`]).stdout.toLowerCase()
    const observedHead = git(repoRoot, ["rev-parse", "--verify", `refs/remotes/${spec.repository.remote}/${spec.repository.headRef}^{commit}`]).stdout.toLowerCase()
    summary.observed_base_sha = observedBase
    summary.observed_head_sha = observedHead
    if (observedBase !== spec.repository.baseSha || observedHead !== spec.repository.headSha) {
      throw assessmentError(`remote authority mismatch (base=${observedBase}; head=${observedHead})`, "STALE")
    }
    ...
    if ((runnerInfo.mode & 0o111) === 0) throw assessmentError("runner.path is not executable", "INFRA_ERROR")
    summary.runner_sha256 = runner.sha256
    for (const path of spec.integrityFiles) {
      const entry = await trackedRegularFile(worktree, path, `integrity file ${path}`)
      summary.integrity[path] = entry.sha256
    }
    ...
    summary.runner.run = { exit: runResult.code, signal: runResult.signal }
    const evidence = await maybeHashEvidence(evidencePath)
    if (evidence) {
      summary.runner_evidence_sha256 = evidence.sha256
      summary.runner_evidence_bytes = evidence.bytes
    }
