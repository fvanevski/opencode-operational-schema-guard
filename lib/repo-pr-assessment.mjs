...
import { mkdir, lstat, open, readFile, readdir, readlink, realpath, rm, rmdir, stat, unlink, writeFile } from "node:fs/promises"
import { createServer } from "node:net"
import { basename, dirname, isAbsolute, join, relative, resolve } from "node:path"
import { fileURLToPath } from "node:url"

export const LOCAL_ASSESSMENT_SCHEMA = "opencode-local-assessment-v1"
export const ASSESSMENT_RESULT_SCHEMA = "opencode-repo-pr-assessment-result-v1"
...
export const ASSESSMENT_REPOSITORY_RUNTIME_ROOT = "/tmp/opencode/verify/repository-owned"
...
const SUPERVISOR_REAP_ERROR = 243
const SUPERVISOR_REPO_ROOT_TOKEN = "__OPENCODE_REPOSITORY_ROOT_CAPABILITY__"
...
const PRESERVED_KERNEL_RESERVATIONS = new Set()
...
export function parseRepoPrAssessmentSpec(value) {
  ...
  if (execution === "gateway-owned") {
    ...
    planArgv = runner.plan_argv === undefined ? undefined : validateTemplateArgv(runner.plan_argv, "runner.plan_argv", { required: ["head_sha", "pr_number", "workspace_root"] })
    runArgv = validateTemplateArgv(runner.run_argv, "runner.run_argv", { required: ["assessment_id", "head_sha", "pr_number", "workspace_root"] })
...
function gitRaw(repoRoot, args, { allowFailure = false } = {}) {
  ...
  if (code !== 0 && !allowFailure) throw assessmentError(`git ${args[0] ?? ""} failed (exit=${code}${stderr.trim() ? `; ${stderr.trim()}` : ""})`, "BLOCKED")
  return { code, stdout, stderr }
}

function gitBuffer(repoRoot, args, { allowFailure = false } = {}) {
  const result = spawnSync("git", args, {
    cwd: repoRoot,
    encoding: null,
    shell: false,
  ...
  if (code !== 0 && !allowFailure) {
    ...
    throw assessmentError(`git ${args[0] ?? ""} failed (exit=${code}${detail ? `; ${detail}` : ""})`, "BLOCKED")
  }
  return { code, stdout, stderr }
}

function git(repoRoot, args, options = {}) {
  const result = gitRaw(repoRoot, args, options)
...
export function assessmentRepositoryRuntimePath(spec, runtimeRoot = ASSESSMENT_REPOSITORY_RUNTIME_ROOT) {
  return join(runtimeRoot, spec.assessmentID)
}
...
function runProcess(executable, argv, cwd, environment, {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const descriptorWriteFds = []
    if (descriptorCwd) supervisedStdio.push(cwdHandle.fd)
    if (gitHandle !== undefined) {
      descriptorWriteFds.push(supervisedStdio.length)
      supervisedStdio.push(gitHandle.fd)
    }
    if (runtimeHandle !== undefined) {
      descriptorWriteFds.push(supervisedStdio.length)
      supervisedStdio.push(runtimeHandle.fd)
    }
    const statusFd = supervised ? supervisedStdio.length : undefined
    if (supervised) supervisedStdio.push("pipe")
    const supervisorArgv = supervised ? [
      RUNNER_SUPERVISOR,
      ...(descriptorCwd ? ["--cwd-fd", "4"] : ["--cwd", cwd]),
      "--status-fd", String(statusFd),
      ...descriptorWriteFds.flatMap((fd) => ["--write-fd", String(fd)]),
    ...
    let statusText = ""
    if (supervised) {
      const stream = child.stdio[statusFd]
      stream.setEncoding("utf8")
      stream.on("data", (chunk) => {
        if (statusText.length <= 4096) statusText += chunk
      })
    }
    child.once("error", rejectPromise)
    child.once("close", (rawCode, processSignal) => {
      if (!supervised) {
        resolvePromise({ code: rawCode ?? 1, signal: processSignal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: false, containmentUncertain: false })
        return
      }
      let status
      try {
        if (statusText.length === 0 || statusText.length > 4096) throw new Error("missing or oversized supervisor status")
        status = JSON.parse(statusText)
        if (status?.version !== 1) throw new Error("unsupported supervisor status version")
      } catch {
        resolvePromise({ code: rawCode ?? 1, signal: processSignal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: true })
        return
      }
      if (status?.kind === "runner" && Number.isInteger(status.code) && status.code >= 0 && status.code <= 255 && processSignal === null && rawCode === status.code) {
        resolvePromise({ code: status.code, signal: null, survivingDescendants: false, controlMutation: false, supervisorError: false, containmentUncertain: false })
        return
      }
      if (status?.kind === "runner-signal" && Number.isInteger(status.signal) && status.signal > 0) {
        resolvePromise({ code: 1, signal: status.signal, survivingDescendants: false, controlMutation: false, supervisorError: false, containmentUncertain: false })
        return
      }
      if (status?.kind === "descendants" && processSignal === null && rawCode === SUPERVISOR_DESCENDANTS) {
        resolvePromise({ code: 0, signal: null, survivingDescendants: true, controlMutation: false, supervisorError: false, containmentUncertain: false })
        return
      }
      if (status?.kind === "control-mutation" && processSignal === null && rawCode === SUPERVISOR_CONTROL_MUTATION) {
        resolvePromise({ code: 0, signal: null, survivingDescendants: false, controlMutation: true, supervisorError: false, containmentUncertain: false })
        return
      }
      if (status?.kind === "setup-error" && processSignal === null && rawCode === SUPERVISOR_SETUP_ERROR) {
        resolvePromise({ code: 1, signal: null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: false })
        return
      }
      if (status?.kind === "reap-error" && processSignal === null && rawCode === SUPERVISOR_REAP_ERROR) {
        resolvePromise({ code: 1, signal: null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: true })
        return
      }
      resolvePromise({ code: rawCode ?? 1, signal: processSignal ?? null, survivingDescendants: false, controlMutation: false, supervisorError: true, containmentUncertain: true })
    })
...
function nulPathBuffers(value) {
  const buffer = Buffer.isBuffer(value) ? value : Buffer.alloc(0)
  const paths = []
  let start = 0
  while (start < buffer.length) {
    const end = buffer.indexOf(0, start)
    if (end < 0) throw assessmentError("Git -z path output was not NUL terminated", "INFRA_ERROR")
    if (end > start) paths.push(Buffer.from(buffer.subarray(start, end)))
    start = end + 1
  }
  return paths
}

function rawOwnerPath(repoRoot, relativePath) {
  if (!Buffer.isBuffer(relativePath) || relativePath.length < 1 || relativePath[0] === 0x2f || relativePath.includes(0)) {
    throw assessmentError("owner preservation received an invalid raw repository path", "INFRA_ERROR")
  }
  let start = 0
  for (let index = 0; index <= relativePath.length; index += 1) {
    if (index !== relativePath.length && relativePath[index] !== 0x2f) continue
    const component = relativePath.subarray(start, index)
    if (component.length === 0 || (component.length === 1 && component[0] === 0x2e) || (component.length === 2 && component[0] === 0x2e && component[1] === 0x2e)) {
      throw assessmentError("owner preservation raw path contains an unsafe component", "INFRA_ERROR")
    }
    start = index + 1
  }
  return Buffer.concat([Buffer.from(`${resolve(repoRoot)}/`), relativePath])
}

function rawPathIdentity(relativePath) {
  return Buffer.from(relativePath).toString("base64")
}

async function hashOwnerPath(repoRoot, relativePath) {
  const requested = rawOwnerPath(repoRoot, relativePath)
  const pathB64 = rawPathIdentity(relativePath)
  let info
  try {
    info = await lstat(requested)
  } catch (error) {
    if (error?.code === "ENOENT") return { path_b64: pathB64, kind: "missing" }
    throw assessmentError(`owner preservation path is unreadable (path_b64=${pathB64}; ${error.code ?? error.message})`, "INFRA_ERROR")
  }
  if (info.isSymbolicLink()) {
    const target = await readlink(requested, { encoding: "buffer" }).catch((error) => {
      throw assessmentError(`owner preservation symlink cannot be read (path_b64=${pathB64}; ${error.code ?? error.message})`, "INFRA_ERROR")
    })
    return { path_b64: pathB64, kind: "symlink", mode: info.mode, size: info.size, sha256: sha256(Buffer.from(target)) }
  }
  if (!info.isFile()) {
    if (info.isDirectory()) throw assessmentError(`owner preservation cannot prove exact bytes for dirty directory entry (path_b64=${pathB64})`, "BLOCKED")
    return { path_b64: pathB64, kind: "other", mode: info.mode, size: info.size }
  }
  const handle = await open(requested, constants.O_RDONLY | constants.O_NOFOLLOW).catch((error) => {
    throw assessmentError(`owner preservation file cannot be opened safely (path_b64=${pathB64}; ${error.code ?? error.message})`, "INFRA_ERROR")
  })
  try {
    ...
    return { path_b64: pathB64, kind: "file", mode: info.mode, size: info.size, sha256: hash.digest("hex") }
  } finally {
    await handle.close()
  }
}
...
async function ownerSnapshot(repoRoot) {
  ...
  const rawStatus = gitBuffer(repoRoot, ["status", "--porcelain=v1", "-z", "--untracked-files=all"]).stdout
  const trackedWorking = nulPathBuffers(gitBuffer(repoRoot, ["diff", "--name-only", "-z", "--no-ext-diff", "--no-renames"]).stdout)
  const trackedIndex = nulPathBuffers(gitBuffer(repoRoot, ["diff", "--cached", "--name-only", "-z", "--no-ext-diff", "--no-renames"]).stdout)
  const untracked = nulPathBuffers(gitBuffer(repoRoot, ["ls-files", "--others", "--exclude-standard", "-z"]).stdout)
  const byIdentity = new Map()
  for (const path of [...trackedWorking, ...trackedIndex, ...untracked]) byIdentity.set(path.toString("hex"), path)
  const paths = [...byIdentity.values()].sort(Buffer.compare)
  const pathState = []
  for (const path of paths) pathState.push(await hashOwnerPath(repoRoot, path))
  const indexDiff = gitBuffer(repoRoot, ["diff", "--cached", "--raw", "-z", "--no-ext-diff", "--no-renames"]).stdout
  const fingerprint = sha256(Buffer.from(JSON.stringify({
    head,
    branch,
    raw_status_sha256: sha256(rawStatus),
    index_diff_sha256: sha256(indexDiff),
    paths: pathState,
  })))
  return { head, branch, status, fingerprint }
}
...
async function trackedRegularFile(worktree, relativePath, label) {
  ...
  if (!inside(canonical, canonicalRoot)) throw assessmentError(`${label} resolves outside the assessment worktree`, "INFRA_ERROR")
...
async function anchorRepositoryOwnedResources(repoRoot, evidenceRoot) {
  ...
  try {
    ...
    try {
      ...
      return { ownerAnchor, evidenceAnchor, reservationAnchor }
    } catch (error) {
...
async function anchorChildDirectory(parentAnchor, relativePath, label) {
  const safe = safeRelativePath(relativePath, label)
  const childPath = anchoredChildPath(parentAnchor, safe, label)
  let handle
  try {
    handle = await open(childPath, constants.O_RDONLY | constants.O_DIRECTORY | constants.O_NOFOLLOW)
  } catch (error) {
    throw assessmentError(`${label} cannot be opened through its admitted parent (${error.code ?? error.message})`, "INFRA_ERROR")
  }
  try {
    const info = await handle.stat()
    if (!info.isDirectory()) throw assessmentError(`${label} must be a directory`, "INFRA_ERROR")
    ...
    const expected = join(parentAnchor.canonical, safe)
    if (canonical !== expected) throw assessmentError(`${label} does not resolve to the entry created under the admitted parent`, "ISOLATION_BREACH")
    return { requested: expected, canonical, dev: info.dev, ino: info.ino, handle, fdPath: `/proc/self/fd/${handle.fd}`, parentRelativePath: safe }
  } catch (error) {
    await handle.close().catch(() => {})
    throw error
  }
...
async function revalidateAnchoredChildDirectory(parentAnchor, childAnchor, relativePath, label) {
  await revalidateDirectoryAnchor(parentAnchor, `${label} parent`)
  const path = anchoredChildPath(parentAnchor, relativePath, label)
  let info
  try {
    info = await lstat(path)
  } catch (error) {
    throw assessmentError(`${label} entry became unavailable (${error.code ?? error.message})`, "ISOLATION_BREACH")
  }
  if (info.isSymbolicLink() || !info.isDirectory() || info.dev !== childAnchor.dev || info.ino !== childAnchor.ino) {
    throw assessmentError(`${label} entry no longer identifies the admitted directory`, "ISOLATION_BREACH")
  }
  const canonical = await realpath(path).catch((error) => {
    throw assessmentError(`${label} entry cannot be resolved (${error.code ?? error.message})`, "ISOLATION_BREACH")
  })
  if (canonical !== childAnchor.canonical) throw assessmentError(`${label} entry canonical identity changed`, "ISOLATION_BREACH")
}

async function removeAnchoredDirectory(parentAnchor, childAnchor, relativePath, label) {
  await revalidateAnchoredChildDirectory(parentAnchor, childAnchor, relativePath, label)
  const entries = await readdir(childAnchor.fdPath)
  for (const entry of entries) {
    await rm(join(childAnchor.fdPath, entry), { recursive: true, force: false })
  }
  await revalidateAnchoredChildDirectory(parentAnchor, childAnchor, relativePath, label)
  try {
    await rmdir(anchoredChildPath(parentAnchor, relativePath, label))
  } catch (error) {
    throw assessmentError(`${label} could not be removed through its admitted parent (${error.code ?? error.message})`, "ISOLATION_BREACH")
  }
  if (await anchoredPathExists(parentAnchor, relativePath)) {
    throw assessmentError(`${label} remained present after descriptor-bound cleanup`, "ISOLATION_BREACH")
  }
}

async function reserveKernelAssessmentID(assessmentID) {
  const label = `opencode-assessment-${assessmentID}`
  const path = `\0${label}`
  const server = createServer((socket) => socket.destroy())
  const reservation = { label, server, error: null }
  ...
  await new Promise((resolvePromise, rejectPromise) => {
    ...
    const onInitialError = (error) => {
      ...
      const kind = error?.code === "EADDRINUSE" ? "BLOCKED" : "INFRA_ERROR"
      rejectPromise(assessmentError(
        error?.code === "EADDRINUSE"
          ? `kernel assessment reservation already exists (${label})`
          : `kernel assessment reservation could not be created (${error.code ?? error.message})`,
        kind,
      ))
    }
...
function revalidateKernelAssessmentReservation(reservation) {
  if (reservation.error) {
    throw assessmentError(`kernel assessment reservation failed (${reservation.error.code ?? reservation.error.message})`, "ISOLATION_BREACH")
  }
  if (!reservation.server.listening) {
    throw assessmentError("kernel assessment reservation is no longer held", "ISOLATION_BREACH")
  }
}

async function releaseKernelAssessmentReservation(reservation) {
  revalidateKernelAssessmentReservation(reservation)
  await new Promise((resolvePromise, rejectPromise) => {
    try {
      reservation.server.close((error) => {
        if (error) rejectPromise(assessmentError(`kernel assessment reservation could not be released (${error.code ?? error.message})`, "ISOLATION_BREACH"))
        else resolvePromise()
      })
    } catch (error) {
      rejectPromise(assessmentError(`kernel assessment reservation could not be released (${error.code ?? error.message})`, "ISOLATION_BREACH"))
    }
  })
}
...
async function reserveAnchoredFile(anchor, relativePath, label) {
  const safe = safeRelativePath(relativePath, label)
  const path = anchoredChildPath(anchor, safe, label)
  let handle
  try {
    handle = await open(path, constants.O_CREAT | constants.O_EXCL | constants.O_RDWR | constants.O_NOFOLLOW, 0o600)
  } catch (error) {
    if (error?.code === "EEXIST") throw assessmentError(`${label} already exists (${relativePath})`, "BLOCKED")
    throw assessmentError(`${label} could not be reserved (${error.code ?? error.message})`, "INFRA_ERROR")
  }
  try {
    const info = await handle.stat()
    if (!info.isFile()) throw assessmentError(`${label} reservation is not a regular file`, "INFRA_ERROR")
    return { relativePath: safe, handle, dev: info.dev, ino: info.ino }
  } catch (error) {
    await handle.close().catch(() => {})
    await unlink(path).catch(() => {})
    throw error
...
async function revalidateAnchoredFile(anchor, reservation, label) {
  let info
  try {
    info = await lstat(anchoredChildPath(anchor, reservation.relativePath, label))
  } catch (error) {
    throw assessmentError(`${label} became unavailable (${error.code ?? error.message})`, "ISOLATION_BREACH")
  }
  if (info.isSymbolicLink() || !info.isFile() || info.dev !== reservation.dev || info.ino !== reservation.ino) {
    throw assessmentError(`${label} no longer identifies the reserved file`, "ISOLATION_BREACH")
  }
}

async function writeReservedAnchoredFile(anchor, reservation, bytes, label) {
  await revalidateAnchoredFile(anchor, reservation, label)
  const payload = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes)
  await reservation.handle.truncate(0)
  let offset = 0
  while (offset < payload.length) {
    const { bytesWritten } = await reservation.handle.write(payload, offset, payload.length - offset, offset)
    if (bytesWritten <= 0) throw assessmentError(`${label} reserved descriptor made no write progress`, "INFRA_ERROR")
    offset += bytesWritten
  }
...
function nativeEvidencePath(runtimePath, spec) {
  return join(runtimePath, "results", spec.assessmentID, "assessment.json")
}
...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  const { ownerAnchor, evidenceAnchor, reservationAnchor } = await anchorRepositoryOwnedResources(repoRoot, evidenceRoot)
  const initialOwner = await ownerSnapshot(ownerAnchor.fdPath)
  const runtimePath = assessmentRepositoryRuntimePath(spec)
  const nativeEvidencePathname = nativeEvidencePath(runtimePath, spec)
  const nativeEvidenceRelative = join("results", spec.assessmentID, "assessment.json")
  ...
  let runnerHandle
  let kernelReservation
  let summaryReservation
  let assessmentIdentityOwned = false
  let nativeReservation
  let preserveRepositoryOwnedState = false
  let returnedSummaryPath = null
  let controlWorktreeCreated = false
  let controlWorktreeAdmitted = false
  let controlRoot
  let controlGitCommon
  let controlParentAnchor
  let controlAnchor
  let controlGitAnchor
  let runtimeParentAnchor
  let runtimeAnchor
  const summary = {
    ...
    repository_runtime_root: runtimePath,
    kernel_assessment_reservation: null,
    kernel_assessment_reservation_cleanup: "NOT_CREATED",
  ...
  try {
    ...
    await revalidateDirectoryAnchor(reservationAnchor, "repository-owned reservation root")
    kernelReservation = await reserveKernelAssessmentID(spec.assessmentID)
    summary.kernel_assessment_reservation = kernelReservation.label
    summary.kernel_assessment_reservation_cleanup = "PENDING"
    summaryReservation = await reserveAnchoredFile(evidenceAnchor, summaryRelative, "outer summary")
    assessmentIdentityOwned = true
    ...
    runtimeParentAnchor = await anchorDirectory(ASSESSMENT_REPOSITORY_RUNTIME_ROOT, "repository-owned runtime parent", { create: true })
    const runtimeEntry = spec.assessmentID
    const runtimeEntryPath = anchoredChildPath(runtimeParentAnchor, runtimeEntry, "repository-owned runtime root")
    try {
      await mkdir(runtimeEntryPath, { mode: 0o700 })
    } catch (error) {
      if (error?.code === "EEXIST") throw assessmentError(`repository-owned runtime path already exists (${runtimePath})`, "BLOCKED")
      throw assessmentError(`repository-owned runtime path could not be reserved (${error.code ?? error.message})`, "INFRA_ERROR")
    }
    runtimeAnchor = await anchorChildDirectory(runtimeParentAnchor, runtimeEntry, "repository-owned runtime root")
    await revalidateAnchoredChildDirectory(runtimeParentAnchor, runtimeAnchor, runtimeEntry, "repository-owned runtime root")
    values.workspace_root = runtimeAnchor.canonical
    ...
    if (initialOwner.head !== authoritySha && spec.runner.authority === "base") {
      const ancestor = git(ownerAnchor.fdPath, ["merge-base", "--is-ancestor", initialOwner.head, authoritySha], { allowFailure: true })
      if (ancestor.code === 1) {
        throw assessmentError(`repository-owned owner checkout ${initialOwner.head} is not an ancestor of pinned base authority ${authoritySha}`, "STALE")
      }
      if (ancestor.code !== 0) {
        throw assessmentError(`repository-owned owner checkout ancestry could not be proven (owner=${initialOwner.head}; base=${authoritySha}; exit=${ancestor.code})`, "INFRA_ERROR")
      }
    }
    ...
    controlParentAnchor = await anchorDirectory(ASSESSMENT_CONTROL_WORKTREE_ROOT, "repository-owned control-snapshot parent", { create: true })
    const controlEntry = basename(controlWorktree)
    const controlEntryPath = anchoredChildPath(controlParentAnchor, controlEntry, "repository-owned control snapshot root")
    try {
      await mkdir(controlEntryPath, { mode: 0o700 })
    } catch (error) {
      if (error?.code === "EEXIST") throw assessmentError(`control snapshot path already exists (${controlWorktree})`, "BLOCKED")
      throw assessmentError(`control snapshot path could not be reserved (${error.code ?? error.message})`, "INFRA_ERROR")
    }
    controlWorktreeCreated = true
    controlAnchor = await anchorChildDirectory(controlParentAnchor, controlEntry, "repository-owned control snapshot root")
    await revalidateAnchoredChildDirectory(controlParentAnchor, controlAnchor, controlEntry, "repository-owned control snapshot root")
    const gatewayControlAlias = `/proc/${process.pid}/fd/${controlAnchor.handle.fd}`
    cloneRepositoryOwnedControlSnapshot(ownerAnchor.fdPath, gatewayControlAlias, authoritySha, spec.repository.remote)
    await revalidateAnchoredChildDirectory(controlParentAnchor, controlAnchor, controlEntry, "repository-owned control snapshot root")
    controlRoot = controlAnchor.canonical
    ...
    controlWorktreeAdmitted = true
    values.repo_root = SUPERVISOR_REPO_ROOT_TOKEN
    ...
    environment.LOCAL_AGENT_ASSESSMENT_ALLOWED_ROOT = runtimeAnchor.canonical
    delete environment.OPENCODE_OPERATION_GUARD_BYPASS
    const writeRoots = ["/dev/null"]
    ...
    if (spec.runner.planArgv) {
      ...
      try {
        planResult = await runProcess(runner.path, planArgv, controlRoot, environment, {
          ...
          runtimeHandle: runtimeAnchor.handle,
          writeRoots,
      ...
      if (planResult.containmentUncertain) {
        preserveRepositoryOwnedState = true
        throw assessmentError("repository-owned runner supervisor could not prove descendant reaping during plan", "INFRA_ERROR")
      }
      ...
      if (afterPlanOwner.head !== initialOwner.head || afterPlanOwner.branch !== initialOwner.branch || afterPlanOwner.status !== initialOwner.status || afterPlanOwner.fingerprint !== initialOwner.fingerprint) {
        throw assessmentError("repository-owned runner plan changed owner checkout exact preservation fingerprint", "ISOLATION_BREACH")
      }
      ...
      await revalidateAnchoredChildDirectory(runtimeParentAnchor, runtimeAnchor, spec.assessmentID, "repository-owned runtime root")
      await revalidateDirectoryAnchor(reservationAnchor, "repository-owned reservation root")
      revalidateKernelAssessmentReservation(kernelReservation)
      await revalidateNativeAssessmentReservation(reservationAnchor, nativeReservation)
      if (await anchoredPathExists(runtimeAnchor, join("results", spec.assessmentID))) throw assessmentError("repository-owned runner plan materialized the reserved native result directory", "ISOLATION_BREACH")
    ...
    revalidateKernelAssessmentReservation(kernelReservation)
    await revalidateNativeAssessmentReservation(reservationAnchor, nativeReservation)
    if (await anchoredPathExists(runtimeAnchor, join("results", spec.assessmentID))) throw assessmentError("repository-owned native result directory appeared before run", "ISOLATION_BREACH")
    await revalidateDirectoryAnchor(ownerAnchor, "owner repository root")
    await revalidateDirectoryAnchor(controlAnchor, "repository-owned control snapshot root")
    await revalidateDirectoryAnchor(controlGitAnchor, "repository-owned control Git metadata")
    ...
    if (runResult.containmentUncertain) {
      preserveRepositoryOwnedState = true
      throw assessmentError("repository-owned runner supervisor could not prove descendant reaping during run", "INFRA_ERROR")
    }
    ...
    await revalidateDirectoryAnchor(controlAnchor, "repository-owned control snapshot root")
    await revalidateDirectoryAnchor(controlGitAnchor, "repository-owned control Git metadata")
    const evidence = await readRepositoryOwnedEvidence(runtimeAnchor, nativeEvidenceRelative, spec, runResult.code)
    ...
    if (nativeReservation) {
      if (preserveRepositoryOwnedState) {
        await nativeReservation.handle.close().catch(() => {})
        summary.native_result_reservation_cleanup = "PRESERVED_CONTAINMENT_UNCERTAIN"
      } else {
        try {
          await revalidateDirectoryAnchor(reservationAnchor, "repository-owned reservation root")
    ...
    if (kernelReservation) {
      if (preserveRepositoryOwnedState) {
        PRESERVED_KERNEL_RESERVATIONS.add(kernelReservation.server)
        summary.kernel_assessment_reservation_cleanup = "PRESERVED_CONTAINMENT_UNCERTAIN"
      } else {
        try {
          await releaseKernelAssessmentReservation(kernelReservation)
          summary.kernel_assessment_reservation_cleanup = "PASS"
        } catch (error) {
          summary.kernel_assessment_reservation_cleanup = "FAILED"
          summary.host_evidence_result = error?.assessmentKind ?? "ISOLATION_BREACH"
          summary.error = String(error?.message ?? error)
        }
      }
      kernelReservation = undefined
    }

    if (controlWorktreeCreated && preserveRepositoryOwnedState) {
      summary.control_snapshot_cleanup = "PRESERVED_CONTAINMENT_UNCERTAIN"
    } else if (controlWorktreeCreated && !controlWorktreeAdmitted) {
      try {
        await controlGitAnchor?.handle.close().catch(() => {})
        controlGitAnchor = undefined
        if (!controlParentAnchor || !controlAnchor) {
          summary.control_snapshot_cleanup = "PRESERVED_UNADMITTED_IDENTITY"
          summary.host_evidence_result = "ISOLATION_BREACH"
          summary.error = "repo-pr-assessment: partial repository-owned control snapshot identity was not admitted; cleanup preserved state"
        } else {
          await removeAnchoredDirectory(controlParentAnchor, controlAnchor, controlAnchor.parentRelativePath, "repository-owned partial control snapshot root")
          summary.control_snapshot_cleanup = "PASS"
        }
      } catch (error) {
        summary.control_snapshot_cleanup = "FAILED"
        summary.host_evidence_result = "ISOLATION_BREACH"
        summary.error = `repo-pr-assessment: partial repository-owned control snapshot cleanup failed (${error.code ?? error.message})`
      }
    } else if (controlWorktreeCreated) {
      try {
        ...
        if (controlHead.code !== 0 || controlHead.stdout.toLowerCase() !== authoritySha || controlStatus.code !== 0 || controlStatus.stdout !== "") {
          ...
          if (!controlParentAnchor || !controlAnchor) throw assessmentError("repository-owned control snapshot cleanup lacks admitted parent/child descriptors", "ISOLATION_BREACH")
          await removeAnchoredDirectory(controlParentAnchor, controlAnchor, controlAnchor.parentRelativePath, "repository-owned control snapshot root")
          summary.control_snapshot_cleanup = "PASS"
        }
      } catch (error) {
        summary.control_snapshot_cleanup = "FAILED"
    ...
    await controlParentAnchor?.handle.close().catch(() => {})
    ...
    } else if (finalOwner.head !== initialOwner.head || finalOwner.branch !== initialOwner.branch || finalOwner.status !== initialOwner.status || finalOwner.fingerprint !== initialOwner.fingerprint) {
      summary.host_evidence_result = "ISOLATION_BREACH"
      summary.error = "repo-pr-assessment: owner workspace exact preservation fingerprint changed during repository-owned assessment"
    }
    try {
      await revalidateDirectoryAnchor(ownerAnchor, "owner repository root")
    } catch (error) {
    ...
    if (summary.host_evidence_result === "PASS" && summary.kernel_assessment_reservation_cleanup !== "PASS") {
      summary.host_evidence_result = "ISOLATION_BREACH"
      summary.error = `repo-pr-assessment: repository-owned kernel assessment reservation cleanup did not complete (${summary.kernel_assessment_reservation_cleanup})`
    }
    for (const [anchor, label] of [[evidenceAnchor, "repository-owned evidence root"], [reservationAnchor, "repository-owned reservation root"]]) {
      try {
    ...
    if (assessmentIdentityOwned) {
      try {
        ...
        summary.native_evidence_path = runtimeAnchor
          ? await recoverAnchoredPath(runtimeAnchor, nativeEvidenceRelative, "native runner evidence")
          : null
      } catch (error) {
      ...
      await writeReservedAnchoredFile(evidenceAnchor, summaryReservation, `${JSON.stringify(summary, null, 2)}\n`, "outer summary").catch((error) => {
        returnedSummaryPath = null
        summary.host_evidence_result = error?.assessmentKind ?? "INFRA_ERROR"
        summary.error = String(error?.message ?? error)
      })
    ...
    await summaryReservation?.handle.close().catch(() => {})
    await runtimeAnchor?.handle.close().catch(() => {})
    await runtimeParentAnchor?.handle.close().catch(() => {})
...
export async function runRepoPrAssessment(specInput, {
  ...
  try {
    ...
    if (spec.runner.planArgv) {
      ...
      summary.runner.plan = { exit: planResult.code, signal: planResult.signal }
      if (planResult.supervisorError) {
        cleanupSafe = false
        throw assessmentError("gateway-owned runner supervisor failed during plan", "INFRA_ERROR")
      }
      if (planResult.controlMutation) {
        cleanupSafe = false
        throw assessmentError("gateway-owned runner supervisor reported a control mutation during plan", "ISOLATION_BREACH")
      }
    ...
    summary.runner.run = { exit: runResult.code, signal: runResult.signal }
    if (runResult.supervisorError) {
      cleanupSafe = false
      throw assessmentError("gateway-owned runner supervisor failed during run", "INFRA_ERROR")
    }
    if (runResult.controlMutation) {
      cleanupSafe = false
      throw assessmentError("gateway-owned runner supervisor reported a control mutation during run", "ISOLATION_BREACH")
    }
    ...
    if (finalOwner.error) {
      ...
      summary.host_evidence_result = "ISOLATION_BREACH"
      summary.error = "repo-pr-assessment: owner workspace exact preservation fingerprint changed during isolated assessment"
    }
    if (summary.host_evidence_result === "PASS" && summary.cleanup_result !== "PASS") {
      summary.host_evidence_result = "ISOLATION_BREACH"
