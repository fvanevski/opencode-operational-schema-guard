...
import { mkdir, lstat, open, readFile, realpath, stat, unlink, writeFile } from "node:fs/promises"
...
function runProcess(executable, argv, cwd, environment, {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const child = spawn(supervised ? "/usr/bin/python3" : executable, supervisorArgv, {
      ...
      env: environment,
      shell: false,
      stdio: supervised ? ["inherit", "inherit", "inherit", executableHandle.fd] : "inherit",
    })
...
async function anchorRepositoryOwnedResources(repoRoot, evidenceRoot) {
  ...
  try {
    ...
    return { ownerAnchor, evidenceAnchor, nativeAnchor }
  } catch (error) {
...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  const { ownerAnchor, evidenceAnchor, nativeAnchor } = await anchorRepositoryOwnedResources(repoRoot, evidenceRoot)
  ...
  try {
    ...
    nativeReservation = await reserveNativeAssessmentID(nativeAnchor, spec.assessmentID)
    ...
    git(ownerAnchor.fdPath, ["worktree", "add", "--detach", controlWorktree, authoritySha])
    ...
    const runner = await pinnedRepositoryFile(controlRoot, authoritySha, spec.runner.path, spec.runner.blobSha, spec.runner.sha256, "runner.path")
    ...
    for (const entry of spec.integrityFiles) {
      const pinned = await pinnedRepositoryFile(controlRoot, authoritySha, entry.path, entry.expectedBlobSha, entry.expectedSha256, `integrity file ${entry.path}`)
    ...
    const writeRoots = [...new Set([dirname(nativeAnchor.canonical), evidenceAnchor.canonical, controlGitCommon, "/dev"])]
    const watchPaths = [spec.runner.path, ...spec.integrityFiles.map((entry) => entry.path)].map((path) => resolve(controlRoot, path))
    ...
    if (spec.runner.planArgv) {
      ...
      await revalidateNativeAssessmentReservation(nativeAnchor, nativeReservation)
      ...
      await revalidatePinnedInputs(controlRoot, authoritySha, spec.runner.path, summary.runner_sha256, spec.runner.blobSha, spec.integrityFiles, summary.integrity, "repository-owned post-plan")
    }
    ...
    await revalidateDirectoryAnchor(ownerAnchor, "owner repository root")
    await revalidatePinnedInputs(controlRoot, authoritySha, spec.runner.path, summary.runner_sha256, spec.runner.blobSha, spec.integrityFiles, summary.integrity, "repository-owned pre-run")
    ...
    if (nativeReservation) {
      try {
        await releaseNativeAssessmentReservation(nativeAnchor, nativeReservation)
    ...
    if (controlWorktreeCreated) {
      try {
        ...
        const controlHead = git(cleanupTarget, ["rev-parse", "HEAD"], { allowFailure: true })
        const controlStatus = git(cleanupTarget, ["status", "--porcelain=v1", "--untracked-files=normal"], { allowFailure: true })
        if (controlHead.code !== 0 || controlHead.stdout.toLowerCase() !== authoritySha || controlStatus.code !== 0 || controlStatus.stdout !== "") {
          ...
          const removed = git(ownerAnchor.fdPath, ["worktree", "remove", "--force", cleanupTarget], { allowFailure: true })
          if (removed.code === 0) {
            git(ownerAnchor.fdPath, ["worktree", "prune"], { allowFailure: true })
            summary.control_snapshot_cleanup = "PASS"
          } else {
            summary.control_snapshot_cleanup = "FAILED"
            summary.host_evidence_result = "ISOLATION_BREACH"
            summary.error = "repo-pr-assessment: repository-owned control snapshot cleanup failed"
          }
        }
      } catch (error) {
        summary.control_snapshot_cleanup = "FAILED"
