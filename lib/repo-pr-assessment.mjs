...
function terminateSurvivingProcessGroup(pid) {
  if (!Number.isInteger(pid) || pid < 1) return false
  try {
    process.kill(-pid, 0)
  } catch (error) {
    if (error?.code === "ESRCH") return false
    throw assessmentError(`could not inspect runner process group (${error.code ?? error.message})`, "INFRA_ERROR")
  }
  try {
    process.kill(-pid, "SIGKILL")
  } catch (error) {
    if (error?.code !== "ESRCH") throw assessmentError(`could not terminate surviving runner descendants (${error.code ?? error.message})`, "INFRA_ERROR")
  }
  return true
}

function runProcess(executable, argv, cwd, environment, { executableHandle, isolateProcessGroup = false } = {}) {
  return new Promise((resolvePromise, rejectPromise) => {
    const descriptorExecution = executableHandle !== undefined
    const child = spawn(descriptorExecution ? "/proc/self/fd/3" : executable, argv, {
      cwd,
      env: environment,
      shell: false,
      detached: isolateProcessGroup,
      stdio: descriptorExecution ? ["inherit", "inherit", "inherit", executableHandle.fd] : "inherit",
    })
    const pid = child.pid
    child.once("error", rejectPromise)
    child.once("exit", (code, signal) => {
      try {
        const survivingDescendants = isolateProcessGroup ? terminateSurvivingProcessGroup(pid) : false
        resolvePromise({ code: code ?? 1, signal: signal ?? null, survivingDescendants })
      } catch (error) {
        rejectPromise(error)
      }
    })
...
async function runRepositoryOwnedRepoPrAssessment(spec, {
  ...
  let runnerHandle
  let returnedSummaryPath = summaryPath
  const summary = {
  ...
  try {
    ...
    if (await anchoredPathExists(nativeAnchor, nativeEvidenceRelative)) throw assessmentError(`native runner evidence path already exists (${nativeEvidencePathname})`, "BLOCKED")
    ...
    if (spec.runner.planArgv) {
      ...
      let planResult
      try {
        planResult = await runProcess(runner.path, planArgv, repoRoot, environment, { executableHandle: runnerHandle, isolateProcessGroup: true })
      } catch (error) {
      ...
      await revalidatePinnedInputs(repoRoot, authoritySha, spec.runner.path, summary.runner_sha256, spec.runner.blobSha, spec.integrityFiles, summary.integrity, "repository-owned post-plan")
    }

    await revalidateDirectoryAnchor(evidenceAnchor, "repository-owned evidence root")
    await revalidateDirectoryAnchor(nativeAnchor, "repository-owned native-result root")
    await revalidatePinnedInputs(repoRoot, authoritySha, spec.runner.path, summary.runner_sha256, spec.runner.blobSha, spec.integrityFiles, summary.integrity, "repository-owned pre-run")
    const runArgv = renderArgv(spec.runner.runArgv, values)
    let runResult
    try {
      runResult = await runProcess(runner.path, runArgv, repoRoot, environment, { executableHandle: runnerHandle, isolateProcessGroup: true })
    } catch (error) {
