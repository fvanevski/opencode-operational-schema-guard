...
export function repositoryIdentityFromRemoteListing(output) {
  const identities = new Set()
  for (const line of String(output ?? "").split(/\r?\n/)) {
    const match = line.trim().match(/^\S+\s+(\S+)\s+\((?:fetch|push)\)$/)
    if (!match) continue
    const identity = canonicalRepositoryIdentity(match[1])
    if (identity) identities.add(identity)
  }
  return identities.size === 1 ? [...identities][0] : undefined
}

export function resolveRepositoryIdentity(directory, { spawn = spawnSync } = {}) {
  let result
  try {
  ...
  return repositoryIdentityFromRemoteListing(result.stdout)
}
