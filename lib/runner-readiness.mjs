...
function assertSecurityOptions(host, seccompProof) {
  ...
  let appliedProfile
  try {
    appliedProfile = JSON.parse(payload)
  } catch (error) {
    blocked(`runner container seccomp SecurityOpt is not valid inline JSON (${error.message})`)
  }
...
export async function verifySeccompProfile(configInput) {
  ...
  let profile
  try {
    profile = JSON.parse(file.bytes.toString("utf8"))
  } catch (error) {
    blocked(`seccomp profile is invalid JSON (${error.message})`)
  }
