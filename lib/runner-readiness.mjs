...
function tmpfsWritable(options) {
  const tokens = options.split(",")
  const rwCount = tokens.filter((token) => token === "rw").length
  const roCount = tokens.filter((token) => token === "ro").length
  if (rwCount + roCount !== 1) blocked("allowed_tmpfs options must contain exactly one explicit rw or ro token")
  return rwCount === 1
}
...
function normalizeSecurityOpt(value) {
  if (value === "no-new-privileges:true" || value === "no-new-privileges=true") return "no-new-privileges"
  return value
}

const LOSSLESS_JSON_NUMBER = Symbol("ghdev-lossless-json-number")

function canonicalJsonNumber(source, label) {
  if (typeof source !== "string" || source.length < 1 || source.length > 128) blocked(`${label} contains an unsupported JSON number literal`)
  const match = source.match(/^(-?)(0|[1-9]\d*)(?:\.(\d+))?(?:[eE]([+-]?\d+))?$/)
  if (!match) blocked(`${label} contains an invalid JSON number literal`)
...
function parseLosslessJson(text, label) {
  ...
  let document
  try {
    document = JSON.parse(text, (_key, value, context) => {
      if (typeof value !== "number") return value
      if (typeof context?.source !== "string") {
        sourceUnavailable = true
        return value
      }
      return { [LOSSLESS_JSON_NUMBER]: canonicalJsonNumber(context.source, label) }
    })
  } catch (error) {
    if (error instanceof RunnerReadinessBlockedError) throw error
    blocked(`${label} is invalid JSON (${error.message})`)
  }
  if (sourceUnavailable) blocked(`${label} cannot be compared losslessly because JSON number source text is unavailable`)
  return document
}

function canonicalJson(value) {
  if (value && typeof value === "object" && Object.hasOwn(value, LOSSLESS_JSON_NUMBER)) return `number:${value[LOSSLESS_JSON_NUMBER]}`
...
function assertSecurityOptions(host, seccompProof) {
  const observed = host.SecurityOpt ?? []
  if (!Array.isArray(observed) || observed.some((entry) => typeof entry !== "string")) blocked("Docker inspect SecurityOpt is invalid")
  const normalized = observed.map(normalizeSecurityOpt)
  if (normalized.filter((entry) => entry === "no-new-privileges").length !== 1) blocked("runner container must enable no-new-privileges exactly once")
  const seccompEntries = observed.filter((entry) => entry.startsWith("seccomp="))
  if (seccompEntries.length !== 1) blocked("runner container must expose exactly one applied seccomp SecurityOpt")
  if (observed.length !== 2) blocked("runner container has an unexpected SecurityOpt entry")
  const payload = seccompEntries[0].slice("seccomp=".length)
  if (payload === "unconfined" || payload === "builtin") blocked("runner container must use the configured custom seccomp profile")
  const appliedProfile = parseLosslessJson(payload, "runner container seccomp SecurityOpt")
  if (!appliedProfile || typeof appliedProfile !== "object" || Array.isArray(appliedProfile)) blocked("runner container seccomp SecurityOpt must contain one JSON object")
  const expectedProfile = seccompProof?.profile
  if (!expectedProfile || typeof expectedProfile !== "object" || Array.isArray(expectedProfile)) blocked("seccomp proof does not contain a parsed profile")
  if (canonicalJson(appliedProfile) !== canonicalJson(expectedProfile)) blocked("runner container applied seccomp profile differs from frozen host profile")
}
...
function assertMounts(config, inspect) {
  ...
  const expectedTmpfs = new Map(config.allowed_tmpfs.map((entry) => [entry.destination, tmpfsWritable(entry.options)]))
  ...
  const observedVolumeDestinations = new Set()
  for (const mount of volumeMounts) {
    const destination = mount?.Destination
    if (typeof destination !== "string" || observedVolumeDestinations.has(destination) || typeof mount?.RW !== "boolean") blocked("runner container named-volume mount evidence is malformed or duplicated")
    observedVolumeDestinations.add(destination)
    const expectedMount = expectedVolumes.get(destination)
    if (!expectedMount || mount?.Name !== expectedMount.name || mount.RW === expectedMount.read_only) blocked("runner container has an unexpected or mismatched named volume")
  }
  for (const destination of expectedVolumes.keys()) if (!observedVolumeDestinations.has(destination)) blocked("runner container named-volume set differs from readiness config")

  const observedTmpfsDestinations = new Set()
  for (const mount of tmpfsMounts) {
    const destination = mount?.Destination
    if (typeof destination !== "string" || observedTmpfsDestinations.has(destination) || typeof mount?.RW !== "boolean") blocked("runner container tmpfs mount evidence is malformed or duplicated")
    observedTmpfsDestinations.add(destination)
    if (!expectedTmpfs.has(destination)) blocked("runner container has an unexpected tmpfs mount")
    if (mount.RW !== expectedTmpfs.get(destination)) blocked("runner container tmpfs writability differs from readiness config")
  }
  for (const destination of expectedTmpfs.keys()) if (!observedTmpfsDestinations.has(destination)) blocked("runner container tmpfs mount set differs from readiness config")
}
...
export function assessRunnerContainer(configInput, inspectInput, { requireRunning = true, volumeInspects = null, seccompProof = null } = {}) {
  ...
  if (!Array.isArray(host.MaskedPaths) || host.MaskedPaths.length !== 0 || !Array.isArray(host.ReadonlyPaths) || host.ReadonlyPaths.length !== 0) blocked("runner container requires explicit empty Docker masked/read-only path lists for systempaths=unconfined")

  assertSecurityOptions(host, seccompProof)
...
export async function verifySeccompProfile(configInput) {
  ...
  const profile = parseLosslessJson(file.bytes.toString("utf8"), "seccomp profile")
  if (!profile || typeof profile !== "object" || Array.isArray(profile)) blocked("seccomp profile must contain one JSON object")
  return { sha256: observed, mtime_ns: file.mtime_ns, ctime_ns: file.ctime_ns, profile }
}
