...
function assertMounts(config, inspect) {
  ...
  if (mounts.length !== volumeMounts.length) blocked("runner container has an unexpected mount type")
...
function assertTmpfs(config, inspect) {
  ...
  if (expected.length === 0 && (observed === null || observed === undefined)) return
  ...
  for (const [destination, options] of normalizedObserved) {
    if (typeof destination !== "string" || typeof options !== "string") blocked("Docker inspect Tmpfs entry is invalid")
  }
