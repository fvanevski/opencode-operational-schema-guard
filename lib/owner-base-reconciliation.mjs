...
export async function reconcileOwnerBase({ specPath, expectedOldSha, expectedBaseSha, expectedTargetSha, cwd = process.cwd() }) {
  for (const [label, value] of [["expected old owner SHA", expectedOldSha], ["expected pinned base SHA", expectedBaseSha], ["expected target SHA", expectedTargetSha]]) {
    if (!LOWER_SHA.test(value ?? "")) throw reconciliationError(`${label} must be 40 lowercase hexadecimal characters`)
  }
  ...
  if (spec.repository.baseSha !== expectedBaseSha) {
    throw reconciliationError(`spec pinned base ${spec.repository.baseSha} does not match expected base ${expectedBaseSha}`, "STALE")
  }
  if (spec.repository.headSha !== expectedTargetSha) {
    throw reconciliationError(`spec target ${spec.repository.headSha} does not match expected target ${expectedTargetSha}`, "STALE")
  }
