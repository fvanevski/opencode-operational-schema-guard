...
export function parseAndValidateConfig(text) {
  ...
  assertRule(String(build.prompt ?? "") === BUILD_AGENT_PROMPT, "Build prompt must exactly match the generated v5.21 prompt.")
  ...
  assertRule(String(verify.prompt ?? "") === VERIFY_AGENT_PROMPT, "Verify prompt must exactly match the generated v5.21 prompt.")
  ...
  assertRule(String(explore.prompt ?? "") === EXPLORE_AGENT_PROMPT, "Explore prompt must exactly match the generated v5.21 prompt.")
  ...
  for (const name of REQUIRED_MODELS) {
    ...
    assertRule(Number.isInteger(model.limit?.context) && model.limit.context >= 196608, `${name} context limit must be at least 196608.`)
    assertRule(Number.isInteger(model.limit?.input) && model.limit.input > 0 && model.limit.input <= 180000, `${name} input limit must be present and no greater than 180000 so native compaction precedes the emergency guard.`)
    assertRule(Number.isInteger(model.limit?.output) && model.limit.output > 0, `${name} output limit must be a positive integer.`)
  }
  return config
}
