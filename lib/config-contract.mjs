...
import { EVIDENCE_ASSESSMENT_RULE, REMEDIATION_AUDIT_RULE } from "./policy-spec.mjs"
...
export function parseAndValidateConfig(text) {
  ...
  assertRule(typeof build.prompt === "string" && build.prompt.trim().length > 0, "Build prompt must be a non-empty string.")
  ...
  assertRule(typeof verify.prompt === "string" && verify.prompt.trim().length > 0, "Verify prompt must be a non-empty string.")
  ...
  assertRule(typeof explore.prompt === "string" && explore.prompt.trim().length > 0, "Explore prompt must be a non-empty string.")
