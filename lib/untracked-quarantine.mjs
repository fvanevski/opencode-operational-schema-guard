...
async function completeQuarantineRemoval(spec, receipt, receiptSha256, options = {}) {
  ...
  if (trackedStateSha256(afterRecords) !== trackedStateSha256(parseStatus(runGit(spec.workspace_root, ["status", "--porcelain=v1", "-z", "--untracked-files=all", "--ignored=matching"], "Git post-quarantine tracked-state confirmation")))) blocked("post-quarantine tracked/staged Git state was unstable during confirmation")
  return {
    result: "PASS",
    action: "quarantine",
    operation_id: spec.operation_id,
