...
export const UNTRACKED_QUARANTINE_OPERATION_ROOT = `${UNTRACKED_QUARANTINE_ROOT}/operations`
const FS_MUTATION_HELPER = fileURLToPath(new URL("../scripts/untracked-quarantine-fs.py", import.meta.url))
const FS_MUTATION_SCHEMA = "opencode-untracked-quarantine-fs-v1"
...
function oneGitLine(workspaceRoot, args, label) {
  ...
  const lines = decodeUTF8(output, label).split(/\r?\n/).filter(Boolean)
...
async function ensureDirectoryTree(path, label) {
  const resolved = resolve(path)
  const parts = resolved.split(sep).filter(Boolean)
  let current = sep
  for (const part of parts) {
    current = join(current, part)
    let info = await lstat(current).catch((error) => error?.code === "ENOENT" ? undefined : Promise.reject(error))
    if (!info) {
      try {
        await mkdir(current, { mode: 0o700 })
      } catch (error) {
        if (error?.code !== "EEXIST") blocked(`${label} cannot be created safely (${error.code ?? error.message})`)
      }
      info = await lstat(current).catch((error) => blocked(`${label} is unavailable after creation (${error.code ?? error.message})`))
    }
    if (!info.isDirectory() || info.isSymbolicLink()) blocked(`${label} traverses a non-directory or symlink at ${current}`)
  }
  await assertDirectoryNotSymlink(resolved, label)
}

async function ensureHarnessRoots() {
  await ensureDirectoryTree(UNTRACKED_QUARANTINE_ROOT, "untracked-quarantine root")
  await ensureDirectoryTree(UNTRACKED_QUARANTINE_SPEC_ROOT, "untracked-quarantine spec root")
  await ensureDirectoryTree(UNTRACKED_QUARANTINE_OPERATION_ROOT, "untracked-quarantine operation root")
}
...
async function workspaceIdentity(workspaceRoot) {
  ...
  const workspaceDev = String(rootStat.dev)
  const workspaceIno = String(rootStat.ino)
  ...
  return { root: workspaceRoot, gitDir, workspaceDev, workspaceIno, sha256: sha256(payload) }
}

function runFilesystemMutation(request, label) {
  const result = spawnSync("/usr/bin/python3", [FS_MUTATION_HELPER], {
    input: `${JSON.stringify({ schema_version: FS_MUTATION_SCHEMA, ...request })}\n`,
    encoding: "utf8",
    env: { PATH: "/usr/bin:/bin", LC_ALL: "C.UTF-8", PYTHONHASHSEED: "0" },
    maxBuffer: 128 * 1024,
    timeout: 30_000,
  })
  if (result.error) blocked(`${label} could not launch descriptor-anchored filesystem helper (${result.error.message})`)
  let document
  try {
    document = JSON.parse(String(result.stdout ?? "").trim())
  } catch {
    blocked(`${label} returned malformed descriptor-anchored filesystem evidence`)
  }
  if (result.status !== 0 || document?.result !== "PASS") {
    const reason = typeof document?.reason === "string" ? document.reason : `exit ${result.status}`
    blocked(`${label} blocked (${reason})`)
  }
  return document
}

function mutationIdentity(identity) {
  return {
    workspace_root: identity.root,
    workspace_dev: identity.workspaceDev,
    workspace_ino: identity.workspaceIno,
  }
}

function inventoryEntriesForPath(receipt, repoPath) {
  return receipt.inventory.entries.filter((entry) => entry.path === repoPath || entry.path.startsWith(`${repoPath}/`))
}
...
async function boundedFileFingerprint(path, counters) {
  let handle
  try {
    handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW)
    const info = await handle.stat()
    if (!info.isFile()) blocked(`inventory file changed type before hashing: ${path}`)
    if (counters.bytes + info.size > MAX_TOTAL_FILE_BYTES) blocked(`inventory exceeds ${MAX_TOTAL_FILE_BYTES} file bytes`)
    ...
    for (;;) {
      ...
      size += bytesRead
      if (counters.bytes + size > MAX_TOTAL_FILE_BYTES) blocked(`inventory exceeds ${MAX_TOTAL_FILE_BYTES} file bytes`)
    ...
    if (error instanceof QuarantineBlockedError) throw error
    blocked(`inventory file cannot be hashed safely (${path}: ${error.code ?? error.message})`)
  } finally {
    await handle?.close().catch(() => {})
  }
}
...
async function inventoryOne(root, repoPath, counters, { allowMissing = false } = {}) {
  ...
  if (info.isFile()) {
    const fingerprint = await boundedFileFingerprint(absolute, counters)
    return [{ path: repoPath, type: "file", mode, size: fingerprint.size, sha256: fingerprint.sha256 }]
  }
  ...
  for (const nameBytes of names) {
    ...
    if (name === "." || name === ".." || name.includes("/") || name.includes("\\")) blocked(`ambiguous or unsupported directory entry under ${repoPath}`)
...
async function writeExclusiveJSON(path, document) {
  ...
  if (bytes.length < 2 || bytes.length > MAX_RECEIPT_BYTES) blocked(`serialized receipt exceeds the ${MAX_RECEIPT_BYTES}-byte recovery limit`)
  let handle
  try {
...
function validateReceiptDocument(receipt) {
  ...
  exactObjectKeys(receipt.quarantine, ["root", "data_path", "capture_root", "inventory_sha256"], "receipt.quarantine")
  if (typeof receipt.quarantine.capture_root !== "string" || !receipt.quarantine.capture_root.startsWith("/") || /[\r\n\0]/.test(receipt.quarantine.capture_root) || resolve(receipt.quarantine.capture_root) !== receipt.quarantine.capture_root) blocked("receipt capture root is invalid")
  if (inside(receipt.quarantine.capture_root, workspaceRoot) || inside(workspaceRoot, receipt.quarantine.capture_root)) blocked("receipt capture root overlaps the governed workspace")
...
function preparedCaptureRoot(spec, identity) {
  const prepared = runFilesystemMutation({
    action: "prepare",
    operation_id: spec.operation_id,
    ...mutationIdentity(identity),
    git_dir: identity.gitDir,
    operation_root: operationDirectory(spec.operation_id),
  }, "quarantine capture preparation")
  const captureRoot = prepared.capture_root
  if (typeof captureRoot !== "string" || !captureRoot.startsWith("/") || resolve(captureRoot) !== captureRoot) blocked("descriptor-anchored helper returned an invalid capture root")
  if (inside(captureRoot, identity.root) || inside(identity.root, captureRoot)) blocked("descriptor-anchored helper returned a capture root that overlaps the governed workspace")
  return captureRoot
}
...
async function stageQuarantine(spec, identity, statusEvidence, sourceInventory) {
  ...
  const captureRoot = preparedCaptureRoot(spec, identity)
  const receipt = {
    ...
    quarantine: { root: opDir, data_path: finalData, capture_root: captureRoot, inventory_sha256: sourceInventory.sha256 },
  }
...
async function removeOriginals(spec, identity, receipt, { afterRemove } = {}) {
  for (const [index, path] of spec.paths.entries()) {
    runFilesystemMutation({
      action: "capture",
      operation_id: spec.operation_id,
      ...mutationIdentity(identity),
      capture_root: receipt.quarantine.capture_root,
      repo_path: path,
      expected_entries: inventoryEntriesForPath(receipt, path),
    }, `descriptor-anchored quarantine capture for ${path}`)
    await afterRemove?.({ path, index })
  }
}
...
async function loadRestoreContext(spec, identity) {
  await assertDirectoryNotSymlink(operationDirectory(spec.operation_id), "quarantine operation directory")
  const loaded = await loadUntrackedQuarantineReceipt(spec.receipt_path, spec.expected_receipt_sha256)
  const receipt = loaded.document
  validateReceiptAgainstSpec(receipt, spec, identity)
  const qData = receipt.quarantine.data_path
  await assertDirectoryNotSymlink(qData, "quarantine data directory")
  for (const path of spec.paths) await assertNoSymlinkAncestors(qData, path)
  ...
  return { loaded, receipt, qData }
}

async function preRestoreState(spec, receipt) {
  const statusBytes = runGit(spec.workspace_root, ["status", "--porcelain=v1", "-z", "--untracked-files=all", "--ignored=matching"], "Git pre-restore dirty-state readback")
  const records = parseStatus(statusBytes)
  for (const record of records) {
    if (spec.paths.some((path) => pathWithin(record.path, path)) && record.status !== "??") {
      blocked("restore destination scope contains non-untracked Git-visible state")
    }
  }
  const tracked = runGit(spec.workspace_root, ["--literal-pathspecs", "ls-files", "-z", "--stage", "--", ...spec.paths], "Git pre-restore tracked/index check")
  if (tracked.length > 0) blocked("restore destination scope is tracked in the current index")
  for (const path of spec.paths) {
    const current = await inventoryPaths(spec.workspace_root, [path], { allowMissing: true })
    if (current.entries.length > 0 && stableJSON(current.entries) !== stableJSON(inventoryEntriesForPath(receipt, path))) {
      blocked(`restore refuses to overwrite or consume later destination content at ${path}`)
    }
  }
  return { trackedSha256: trackedStateSha256(records) }
}

async function verifyRestoreState(spec, receipt, expectedTrackedSha256) {
  ...
  if (after.pathsSha256 !== receipt.status.paths_sha256) {
    blocked("restored Git path fingerprints do not reproduce the exact pre-quarantine requested-path state")
  }
  if (expectedTrackedSha256 !== undefined && expectedTrackedSha256 !== trackedStateSha256(after.records)) {
    blocked("restore changed tracked/staged Git state")
  }
  return { statusSha256: after.statusSha256, pathsSha256: after.pathsSha256 }
}

export async function preflightUntrackedQuarantineRestore(specInput) {
  const spec = validateSpecDocument(specInput)
  if (spec.action !== "restore") blocked("restore preflight requires a restore spec")
  await ensureHarnessRoots()
  const identity = await workspaceIdentity(spec.workspace_root)
  const { loaded, receipt } = await loadRestoreContext(spec, identity)
  const before = await preRestoreState(spec, receipt)
  return {
    workspace_sha256: identity.sha256,
    receipt_sha256: loaded.sha256,
    inventory_sha256: receipt.inventory.sha256,
    tracked_state_sha256: before.trackedSha256,
  }
}

export async function verifyUntrackedQuarantineRestore(specInput, expectedTrackedSha256) {
  const spec = validateSpecDocument(specInput)
  if (spec.action !== "restore") blocked("restore verification requires a restore spec")
  await ensureHarnessRoots()
  const identity = await workspaceIdentity(spec.workspace_root)
  const { loaded, receipt } = await loadRestoreContext(spec, identity)
  const verified = await verifyRestoreState(spec, receipt, expectedTrackedSha256)
  return {
    workspace_sha256: identity.sha256,
    receipt_sha256: loaded.sha256,
    inventory_sha256: receipt.inventory.sha256,
    status_sha256: verified.statusSha256,
    paths_sha256: verified.pathsSha256,
  }
}

async function restoreFromReceipt(spec, identity) {
  const { loaded, receipt, qData } = await loadRestoreContext(spec, identity)
  const before = await preRestoreState(spec, receipt)
  for (const path of spec.paths) {
    runFilesystemMutation({
      action: "restore",
      ...
      ...mutationIdentity(identity),
      capture_root: receipt.quarantine.capture_root,
      quarantine_root: qData,
      operation_root: operationDirectory(spec.operation_id),
      repo_path: path,
      expected_entries: inventoryEntriesForPath(receipt, path),
    }, `descriptor-anchored restore for ${path}`)
  }
  const verified = await verifyRestoreState(spec, receipt, before.trackedSha256)
  return {
    receipt,
    receiptSha256: loaded.sha256,
    statusSha256: verified.statusSha256,
    pathsSha256: verified.pathsSha256,
  }
}

async function existingQuarantineReceipt(spec, identity) {
  const path = receiptPath(spec.operation_id)
  const info = await lstat(path).catch((error) => error?.code === "ENOENT" ? undefined : Promise.reject(error))
  if (!info) return undefined
  await assertDirectoryNotSymlink(operationDirectory(spec.operation_id), "quarantine operation directory")
  const loaded = await loadUntrackedQuarantineReceipt(path)
  validateReceiptAgainstSpec(loaded.document, spec, identity)
  const expectedCaptureRoot = preparedCaptureRoot(spec, identity)
  if (loaded.document.quarantine.capture_root !== expectedCaptureRoot) blocked("existing quarantine receipt capture root does not match the deterministic helper-selected location")
  const qData = loaded.document.quarantine.data_path
  await assertDirectoryNotSymlink(qData, "quarantine data directory")
  for (const requested of spec.paths) await assertNoSymlinkAncestors(qData, requested)
  const quarantineInventory = await inventoryPaths(qData, spec.paths)
  if (quarantineInventory.sha256 !== loaded.document.inventory.sha256) blocked("existing quarantine bytes/inventory no longer match the receipt")
  return loaded
}

export async function verifyUntrackedQuarantineCompletion(specInput) {
  const spec = validateSpecDocument(specInput)
  if (spec.action !== "quarantine") blocked("quarantine completion verification requires a quarantine spec")
  await ensureHarnessRoots()
  const identity = await workspaceIdentity(spec.workspace_root)
  if (identity.sha256 !== spec.expected_workspace_sha256) blocked("workspace identity fingerprint changed after inspection")
  const existing = await existingQuarantineReceipt(spec, identity)
  if (!existing) blocked("completed quarantine receipt is missing")
  ...
  if (sha256(afterBytes) !== existing.document.status.expected_after_sha256) blocked("post-quarantine dirty-state fingerprint differs from the precomputed expected state")
  ...
  return {
    ...
    inventory_sha256: existing.document.inventory.sha256,
    status_sha256: sha256(afterBytes),
  }
}

async function completeQuarantineRemoval(spec, identity, receipt, receiptSha256, options = {}) {
  await removeOriginals(spec, identity, receipt, options)
  const afterBytes = runGit(spec.workspace_root, ["status", "--porcelain=v1", "-z", "--untracked-files=all", "--ignored=matching"], "Git post-quarantine readback")
  const afterRecords = parseStatus(afterBytes)
...
export async function runUntrackedQuarantine(specInput, options = {}) {
  const spec = validateSpecDocument(specInput)
  await ensureHarnessRoots()
  const identity = await workspaceIdentity(spec.workspace_root)
  ...
  if (spec.action === "quarantine") {
    if (identity.sha256 !== spec.expected_workspace_sha256) blocked("workspace identity fingerprint changed after inspection")
    const existing = await existingQuarantineReceipt(spec, identity)
    if (existing) return completeQuarantineRemoval(spec, identity, existing.document, existing.sha256, options)
    const evidence = statusAndPathEvidence(spec.workspace_root, spec.paths)
    ...
    if (sourceInventory.sha256 !== spec.expected_inventory_sha256) blocked("source inventory fingerprint changed after inspection")
    const { receipt, receiptIdentity } = await stageQuarantine(spec, identity, evidence, sourceInventory)
    return completeQuarantineRemoval(spec, identity, receipt, receiptIdentity.sha256, options)
  }
