...
async function stageQuarantine(spec, identity, statusEvidence, sourceInventory) {
  ...
  const prepared = runFilesystemMutation({
    ...
    operation_root: opDir,
  }, "quarantine capture preparation")
  if (typeof prepared.capture_root !== "string" || !prepared.capture_root.startsWith("/")) blocked("descriptor-anchored helper returned an invalid capture root")
  const receipt = {
    ...
    quarantine: { root: opDir, data_path: finalData, capture_root: prepared.capture_root, inventory_sha256: sourceInventory.sha256 },
  }
