...
async function removeOriginals(spec, receipt, { afterRemove } = {}) {
  const expected = entryMap(receipt.inventory.entries)
  for (const [index, path] of spec.paths.entries()) {
    await assertNoSymlinkAncestors(spec.workspace_root, path)
    ...
    if (current.entries.length === 0) continue
...
async function completeQuarantineRemoval(spec, identity, receipt, receiptSha256, options = {}) {
  await removeOriginals(spec, receipt, options)
