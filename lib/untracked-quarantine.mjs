...
function validateSpecDocument(document) {
  ...
  const extra = action === "inspect"
    ? []
    : action === "quarantine"
      ? ["expected_workspace_sha256", "expected_status_sha256", "expected_paths_sha256"]
  ...
  if (action === "quarantine") {
    for (const field of ["expected_workspace_sha256", "expected_status_sha256", "expected_paths_sha256"]) {
...
export async function loadUntrackedQuarantineSpec(path) {
  ...
  if (!resolved.endsWith(".json") || !inside(resolved, UNTRACKED_QUARANTINE_SPEC_ROOT) || /[*?\[\]{}]/.test(resolved)) {
    blocked(`spec must be one concrete .json file under ${UNTRACKED_QUARANTINE_SPEC_ROOT}`)
  }
  let handle
  try {
...
async function assertDirectoryNotSymlink(path, label) {
  const info = await lstat(path).catch((error) => blocked(`${label} is unavailable (${error.code ?? error.message})`))
...
async function readReceipt(path, expectedSha256) {
  let handle
  try {
    handle = await open(path, constants.O_RDONLY | constants.O_NOFOLLOW)
    const info = await handle.stat()
    if (!info.isFile() || info.size < 2 || info.size > MAX_RECEIPT_BYTES) blocked("receipt must be a bounded regular non-symlink file")
    const bytes = await handle.readFile()
    if (sha256(bytes) !== expectedSha256) blocked("receipt SHA-256 does not match the restore spec")
    let document
    try {
      document = JSON.parse(bytes.toString("utf8"))
    } catch (error) {
    ...
    if (document?.schema_version !== UNTRACKED_QUARANTINE_RECEIPT_SCHEMA) blocked("receipt schema is invalid")
    return { document, bytes, sha256: expectedSha256 }
  } catch (error) {
    if (error instanceof QuarantineBlockedError) throw error
...
function validateReceiptAgainstSpec(receipt, spec, identity) {
  if (
    ...
    || receipt.quarantine?.data_path !== dataPath(spec.operation_id)
    || !Array.isArray(receipt.inventory?.entries)
    || !SHA256.test(receipt.inventory?.sha256 ?? "")
    || inventoryHash(receipt.inventory.entries) !== receipt.inventory.sha256
  ) blocked("receipt does not match the exact workspace, authority, path set, or inventory declared by the spec")
}
