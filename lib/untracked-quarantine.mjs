...
function statusAndPathEvidenceForRestore(workspaceRoot, paths) {
  ...
  return { statusSha256: sha256(statusBytes) }
}
...
async function restoreFromReceipt(spec, identity) {
  ...
  if (before.statusSha256 !== receipt.status.expected_after_sha256) blocked("pre-restore dirty-state fingerprint no longer matches the exact post-quarantine state")
