...
import { ASSESSMENT_EVIDENCE_ROOT, loadAssessmentSpec } from "./repo-pr-assessment.mjs"
...
async function assessmentEvidence(output) {
  ...
  const summary = await readAssessmentSummary(marker[6])
  if (
    !summary
    || summary.schema_version !== "opencode-repo-pr-assessment-result-v1"
...
function ownerBaseStaleCause(evidence, pending, authority) {
  const summary = evidence?.summary
  const expectedError = `repo-pr-assessment: repository-owned owner checkout is ${authority?.observedHead}, not pinned base authority ${pending.baseSha}`
  return Boolean(
    evidence?.result === "STALE"
    && pending.reconcilable
    && SHA40.test(authority?.observedHead ?? "")
    && summary?.runner_execution === "repository-owned"
    && summary?.runner_authority === "base"
    && summary?.base_ref === pending.baseRef
    && summary?.owner_initial?.head === authority.observedHead
    && summary?.owner_initial?.branch === pending.baseRef
    && summary?.owner_initial?.status === ""
      ...
      if (assessment) {
        ...
        if (pending) {
          ...
          if (evidence.result === "STALE") {
            if (ownerBaseStaleCause(evidence, pending, authority)) {
              ...
              await clearLifecycle(stateDirectory, directory)
              appendLifecycleNotice(output, `OPERATIONAL_TARGET_RECONCILIATION: not-admitted; STALE did not prove the exact clean-owner-behind-pinned-base condition; target=${pending.targetSha}`)
            }
          } else {
            await clearLifecycle(stateDirectory, directory)
          }
