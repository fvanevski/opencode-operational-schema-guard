...
import { createOperationGuard as createCoreOperationGuard, normalizeExactWrapperContinuations } from "./operation-guard-core.mjs"
import { ASSESSMENT_EVIDENCE_ROOT, ASSESSMENT_RESULT_SCHEMA, loadAssessmentSpec } from "./repo-pr-assessment.mjs"
import {
  loadUntrackedQuarantineReceipt,
  loadUntrackedQuarantineSpec,
  preflightUntrackedQuarantineRestore,
  receiptPath,
  runUntrackedQuarantine,
  verifyUntrackedQuarantineCompletion,
  verifyUntrackedQuarantineRestore,
  UNTRACKED_QUARANTINE_OPERATION_ROOT,
} from "./untracked-quarantine.mjs"
...
const UNTRACKED_QUARANTINE_PATH = "/home/filip/.config/opencode/plugins/operational-schema-v5/scripts/untracked-quarantine.mjs"
const QUARANTINE_LEDGER_SCHEMA = "opencode-untracked-quarantine-ledger-v1"
const MAX_QUARANTINE_LEDGER_OPERATIONS = 256
...
function assessmentInvocation(command) {
  ...
  if (/[\r\n]/.test(continued)) return undefined
  const stripped = stripRtk(continued)
  if (!stripped.startsWith(LOCAL_ASSESSMENT_RUNNER_PATH)) return undefined
...
function reconciliationInvocation(command) {
  ...
  if (/[\r\n]/.test(continued)) return undefined
  const stripped = stripRtk(continued)
  if (!stripped.startsWith(OWNER_BASE_RECONCILIATION_PATH)) return undefined
  const parts = stripped.split(/\s+/)
  if (
    parts.length !== 9
...
function untrackedQuarantineInvocation(command) {
  const stripped = String(command ?? "").trim()
  if (!stripped.startsWith(UNTRACKED_QUARANTINE_PATH) || /[\r\n]/.test(stripped)) return undefined
  const parts = stripped.split(/\s+/)
  if (parts.length !== 3 || parts[0] !== UNTRACKED_QUARANTINE_PATH || parts[1] !== "--spec") return undefined
  return { specPath: parts[2] }
}

function mentionsUntrackedQuarantine(command) {
  return /(?:^|[\\/\s])(?:untracked-quarantine\.mjs|untracked-quarantine-fs\.py)(?:\s|$)/.test(String(command ?? ""))
}
...
function quarantineLedgerStatePath(stateDirectory, directory) {
  return join(resolve(stateDirectory), `${stateKey(directory)}.untracked-quarantine.json`)
}
...
async function readCoreAuthority(stateDirectory, directory) {
  ...
  return {
    ...
    epoch: Number.isSafeInteger(value.authorityEpoch) && value.authorityEpoch >= 0 ? value.authorityEpoch : undefined,
...
function validateQuarantineLedger(value, directory) {
  if (!value || typeof value !== "object" || Array.isArray(value) || value.schema_version !== QUARANTINE_LEDGER_SCHEMA || value.workspace_root !== resolve(directory) || !Array.isArray(value.operations) || value.operations.length > MAX_QUARANTINE_LEDGER_OPERATIONS) {
    throw new Error("Operational schema guard: persisted untracked-quarantine ledger is invalid; typed recovery remains fail-closed")
  }
  const ids = new Set()
  for (const entry of value.operations) {
    if (
      !entry || typeof entry !== "object" || Array.isArray(entry)
      || typeof entry.operation_id !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(entry.operation_id) || ids.has(entry.operation_id)
      || !new Set(["inspected", "quarantined", "restored"]).has(entry.phase)
      || entry.workspace_root !== resolve(directory) || !SHA256.test(entry.workspace_sha256 ?? "")
      || !entry.authority || !new Set(["target", "strict-start"]).has(entry.authority.mode) || !SHA40.test(entry.authority.binding ?? "")
      || !Array.isArray(entry.paths) || entry.paths.length < 1 || entry.paths.length > 32 || entry.paths.some((path) => typeof path !== "string")
      || !SHA256.test(entry.status_sha256 ?? "") || !SHA256.test(entry.paths_sha256 ?? "") || !SHA256.test(entry.expected_after_status_sha256 ?? "") || !SHA256.test(entry.inventory_sha256 ?? "")
      || (entry.phase !== "inspected" && (entry.receipt_path !== receiptPath(entry.operation_id) || !SHA256.test(entry.receipt_sha256 ?? "")))
      || (entry.phase === "inspected" && (entry.receipt_path !== null || entry.receipt_sha256 !== null))
    ) throw new Error("Operational schema guard: persisted untracked-quarantine ledger entry is invalid; typed recovery remains fail-closed")
    ids.add(entry.operation_id)
  }
  return value
}

async function readQuarantineLedger(stateDirectory, directory) {
  const missing = { schema_version: QUARANTINE_LEDGER_SCHEMA, workspace_root: resolve(directory), operations: [] }
  const value = await readJSON(quarantineLedgerStatePath(stateDirectory, directory), "untracked-quarantine ledger", { missing })
  return validateQuarantineLedger(value, directory)
}

async function writeQuarantineLedger(stateDirectory, directory, ledger) {
  validateQuarantineLedger(ledger, directory)
  const root = resolve(stateDirectory)
  await mkdir(root, { recursive: true, mode: 0o700 })
  const path = quarantineLedgerStatePath(stateDirectory, directory)
  const temporary = `${path}.${process.pid}.${randomUUID()}.tmp`
  let handle
  try {
    handle = await open(temporary, "wx", 0o600)
...
function sameQuarantineIdentity(entry, spec) {
  return Boolean(
    entry
    && entry.workspace_root === resolve(spec.workspace_root)
    && entry.authority.mode === spec.authority.mode
    && entry.authority.binding === spec.authority.binding
    && JSON.stringify(entry.paths) === JSON.stringify(spec.paths)
  )
}

function exactQuarantinePass(output) {
  const matches = [...String(output?.output ?? "").matchAll(/^UNTRACKED_QUARANTINE_RESULT=PASS\s*$/gm)]
  return Number(output?.metadata?.exit) === 0 && matches.length === 1
}
...
function ownerBaseStaleOwnerSha(evidence, pending, authority) {
  ...
  if (observedHead !== undefined && !SHA40.test(observedHead)) return undefined
  if (observedHead !== undefined && observedHead !== ownerSha) return undefined
...
export function createOperationGuard(options = {}) {
  ...
  const stateRoot = stateDirectory ? resolve(stateDirectory) : undefined
  const workspaceInsideStateRoot = Boolean(stateRoot && inside(directory, stateRoot))
  const protectedMutationRoots = stateDirectory
    ? [
        ...(options.protectedMutationRoots ?? []),
        resolve(UNTRACKED_QUARANTINE_OPERATION_ROOT),
        ...(!workspaceInsideStateRoot ? [stateRoot] : []),
      ]
    : options.protectedMutationRoots
  const protectedMutationPaths = stateDirectory
    ? [
        ...(options.protectedMutationPaths ?? []),
        lifecycleStatePath(stateDirectory, directory),
        quarantineLedgerStatePath(stateDirectory, directory),
      ]
    : options.protectedMutationPaths
  const core = createCoreOperationGuard({ ...options, protectedMutationRoots, protectedMutationPaths })
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      const authorityBefore = await readCoreAuthority(stateDirectory, directory)
      await core["chat.message"]?.(input, output)
      const authority = await readCoreAuthority(stateDirectory, directory)
      if (authorityBefore?.epoch !== authority?.epoch || authorityBefore?.binding !== authority?.binding || authorityBefore?.mode !== authority?.mode) {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      const quarantine = untrackedQuarantineInvocation(command)
      ...
      if (mentionsUntrackedQuarantine(command) && !quarantine) {
        throw new Error(`Operational schema guard: typed untracked recovery must use exactly ${UNTRACKED_QUARANTINE_PATH} --spec /tmp/opencode/verify/untracked-quarantine/specs/<file>.json with no wrapper, prefix, extra argv, or shell syntax.`)
      }

      if (quarantine) {
        const loaded = await loadUntrackedQuarantineSpec(quarantine.specPath).catch((error) => {
          throw new Error(`Operational schema guard: untracked-quarantine spec preflight failed (${error.message})`)
        })
        const spec = loaded.spec
        if (resolve(spec.workspace_root) !== resolve(directory)) {
          throw new Error("Operational schema guard: untracked-quarantine workspace does not match the governed workspace.")
        }
        const authorityMatches = authority?.mode === spec.authority.mode && authority?.binding === spec.authority.binding
        if (spec.action === "inspect" || spec.action === "quarantine") {
          if (!authorityMatches || !new Set(["pending", "mismatch"]).has(authority?.status)) {
            throw new Error("Operational schema guard: inspect/quarantine is admitted only for the exact persisted pending or mismatched authority binding in the governed workspace.")
          }
          if (lifecycle) {
            throw new Error("Operational schema guard: untracked quarantine cannot mutate a workspace while authenticated OWNER_RECONCILIATION is active.")
          }
        } else {
          if (authority?.binding && !authorityMatches) {
            throw new Error("Operational schema guard: restore receipt authority conflicts with the currently persisted workspace authority.")
          }
          if (lifecycle) {
            throw new Error("Operational schema guard: receipt restore cannot mutate a workspace while authenticated OWNER_RECONCILIATION is active.")
          }
        }

        const ledger = await readQuarantineLedger(stateDirectory, directory)
        const entry = ledger.operations.find((candidate) => candidate.operation_id === spec.operation_id)
        if (spec.action === "inspect") {
          if (entry) throw new Error(`Operational schema guard: untracked-quarantine operation_id ${spec.operation_id} was already authenticated and cannot be reused.`)
          if (ledger.operations.length >= MAX_QUARANTINE_LEDGER_OPERATIONS) throw new Error("Operational schema guard: untracked-quarantine ledger is full; do not discard recovery history implicitly.")
          const inspection = await runUntrackedQuarantine(spec).catch((error) => {
            throw new Error(`Operational schema guard: independent untracked-quarantine inspection failed (${error.message})`)
          })
          pendingQuarantines.set(key(input), { action: "inspect", spec, inspection, authorityEpoch: authority?.epoch })
        } else if (spec.action === "quarantine") {
          if (!sameQuarantineIdentity(entry, spec) || entry.phase === "restored") throw new Error("Operational schema guard: quarantine requires the same authenticated prior inspect operation and exact workspace/authority/path identity.")
          if (
            spec.expected_workspace_sha256 !== entry.workspace_sha256
            || spec.expected_status_sha256 !== entry.status_sha256
            || spec.expected_paths_sha256 !== entry.paths_sha256
            || spec.expected_inventory_sha256 !== entry.inventory_sha256
          ) throw new Error("Operational schema guard: quarantine fingerprints do not match the authenticated prior inspect evidence.")
          pendingQuarantines.set(key(input), { action: "quarantine", spec, authorityEpoch: authority?.epoch })
        } else {
          if (!sameQuarantineIdentity(entry, spec) || entry.phase !== "quarantined") throw new Error("Operational schema guard: restore requires one authenticated completed quarantine operation.")
          if (spec.receipt_path !== entry.receipt_path || spec.expected_receipt_sha256 !== entry.receipt_sha256) throw new Error("Operational schema guard: restore receipt path/digest do not match the authenticated completed quarantine receipt.")
          const restorePreflight = await preflightUntrackedQuarantineRestore(spec).catch((error) => {
            throw new Error(`Operational schema guard: restore preflight failed (${error.message})`)
          })
          if (restorePreflight.receipt_sha256 !== entry.receipt_sha256 || restorePreflight.inventory_sha256 !== entry.inventory_sha256) {
            throw new Error("Operational schema guard: restore preflight does not match the authenticated quarantine ledger.")
          }
          pendingQuarantines.set(key(input), { action: "restore", spec, restorePreflight, authorityEpoch: authority?.epoch })
        }
      }
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (quarantine) {
        const pending = pendingQuarantines.get(key(input))
        pendingQuarantines.delete(key(input))
        if (pending && exactQuarantinePass(output)) {
          const authority = await readCoreAuthority(stateDirectory, directory)
          if (!Number.isSafeInteger(pending.authorityEpoch) || authority?.epoch !== pending.authorityEpoch) {
            throw new Error(`Operational schema guard: untracked-quarantine completion is stale because authority epoch changed from ${pending.authorityEpoch ?? "unknown"} to ${authority?.epoch ?? "unknown"}; result remains unauthenticated.`)
          }
          const ledger = await readQuarantineLedger(stateDirectory, directory)
          const existingIndex = ledger.operations.findIndex((entry) => entry.operation_id === pending.spec.operation_id)
          if (pending.action === "inspect") {
            if (existingIndex >= 0) throw new Error("Operational schema guard: authenticated untracked-quarantine operation_id changed concurrently during inspect.")
            const inspection = pending.inspection
            ledger.operations.push({
              operation_id: pending.spec.operation_id,
              phase: "inspected",
              workspace_root: resolve(pending.spec.workspace_root),
              workspace_sha256: inspection.workspace_sha256,
              authority: pending.spec.authority,
              paths: pending.spec.paths,
              status_sha256: inspection.status_sha256,
              paths_sha256: inspection.paths_sha256,
              expected_after_status_sha256: inspection.expected_after_status_sha256,
              inventory_sha256: inspection.inventory_sha256,
              receipt_path: null,
              receipt_sha256: null,
            })
          } else if (pending.action === "quarantine") {
            if (existingIndex < 0) throw new Error("Operational schema guard: authenticated untracked-quarantine inspect ledger disappeared before quarantine completion.")
            const entry = ledger.operations[existingIndex]
            const loadedReceipt = await loadUntrackedQuarantineReceipt(receiptPath(pending.spec.operation_id))
            const receipt = loadedReceipt.document
            if (
              !sameQuarantineIdentity(entry, pending.spec)
              || receipt.operation_id !== entry.operation_id
              || receipt.workspace.root !== entry.workspace_root
              || receipt.workspace.sha256 !== entry.workspace_sha256
              || receipt.authority.mode !== entry.authority.mode
              || receipt.authority.binding !== entry.authority.binding
              || JSON.stringify(receipt.paths) !== JSON.stringify(entry.paths)
              || receipt.status.before_sha256 !== entry.status_sha256
              || receipt.status.paths_sha256 !== entry.paths_sha256
              || receipt.status.expected_after_sha256 !== entry.expected_after_status_sha256
              || receipt.inventory.sha256 !== entry.inventory_sha256
            ) throw new Error("Operational schema guard: completed quarantine receipt does not match the authenticated prior inspect ledger.")
            const completion = await verifyUntrackedQuarantineCompletion(pending.spec).catch((error) => {
              throw new Error(`Operational schema guard: quarantine completion readback failed (${error.message})`)
            })
            if (completion.receipt_sha256 !== loadedReceipt.sha256 || completion.inventory_sha256 !== entry.inventory_sha256) {
              throw new Error("Operational schema guard: quarantine completion readback does not match the authenticated receipt/ledger.")
            }
            ledger.operations[existingIndex] = { ...entry, phase: "quarantined", receipt_path: receiptPath(entry.operation_id), receipt_sha256: loadedReceipt.sha256 }
          } else {
            if (existingIndex < 0 || ledger.operations[existingIndex].phase !== "quarantined") throw new Error("Operational schema guard: authenticated quarantine ledger changed before restore completion.")
            const entry = ledger.operations[existingIndex]
            const completion = await verifyUntrackedQuarantineRestore(pending.spec, pending.restorePreflight?.tracked_state_sha256).catch((error) => {
              throw new Error(`Operational schema guard: restore completion readback failed (${error.message})`)
            })
            if (completion.receipt_sha256 !== entry.receipt_sha256 || completion.inventory_sha256 !== entry.inventory_sha256) {
              throw new Error("Operational schema guard: restore completion readback does not match the authenticated quarantine ledger.")
            }
            ledger.operations[existingIndex] = { ...entry, phase: "restored" }
          }
          await writeQuarantineLedger(stateDirectory, directory, ledger)
        }
      }
