...
export function createOperationGuard(options = {}) {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      if (authorityBefore?.binding !== authority?.binding || authorityBefore?.mode !== authority?.mode) {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (quarantine) {
        ...
        if (spec.action === "inspect") {
          ...
          pendingQuarantines.set(key(input), { action: "inspect", spec, inspection })
          ...
          pendingQuarantines.set(key(input), { action: "quarantine", spec })
        } else {
          ...
          pendingQuarantines.set(key(input), { action: "restore", spec, restorePreflight })
        }
      }
