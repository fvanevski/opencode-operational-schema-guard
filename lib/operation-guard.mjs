...
function targetRecoveryGuidance(binding) {
  return `For repository PR host evidence, run ${LOCAL_ASSESSMENT_RUNNER_PATH} --spec <absolute-spec-path>; the gateway owns exact-head fetch, isolation, proof, and cleanup without moving the owner checkout. If a repository-owned base-authority assessment is STALE only because the clean owner base branch is behind its pinned base, use ${OWNER_BASE_RECONCILIATION_PATH} --spec <absolute-spec-path> --expected-old-sha <40-lowercase-owner-sha>; that typed helper alone may fast-forward the owner to the spec-pinned base after fresh exact authority checks. For caller-owned validation in an authorized disposable worktree, run one bare git worktree add --detach <absolute-disposable-path> ${binding}, set subsequent tool workdir to that path, and run one separate bare git rev-parse HEAD proof there. Use git switch --detach only when the user explicitly permits moving the current checkout.`
}
...
function ownerBaseReconciliationCommandError(command) {
  ...
  if (
    ...
    return `Owner-base reconciliation must use exactly ${OWNER_BASE_RECONCILIATION_PATH} --spec ${LOCAL_ASSESSMENT_SPEC_ROOT}/<file>.json --expected-old-sha <40-lowercase-sha>.`
  }
  return undefined
}
...
function ownerBaseReconciliationInvocation(command) {
  ...
  const parts = normalized.split(/\s+/)
  return { specPath: parts[2], expectedOldSha: parts[4] }
}
...
export function createOperationGuard({ directory = process.cwd(), env = process.env, policy = policyFromEnv(env), stateDirectory, client } = {}) {
  ...
  return {
    ...
    "experimental.chat.system.transform": async (input, output) => {
      try {
        ...
        if (PRIMARY_AGENTS.has(state.agent)) {
          ...
          if (state.lastInputTokens >= policy.primaryContextWarningTokens && !state.contextNoticeSent) {
            output.system.push(`OPERATIONAL GUARD: primary input context is ${state.lastInputTokens} tokens. Remain in this session and yield normally so OpenCode can compact and auto-continue. Do not write a session-close handoff and do not ask the user to start another primary session. Route only already-bounded noisy work while waiting for compaction.`)
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (PRIMARY_AGENTS.has(state.agent) && state.lastInputTokens >= policy.primaryContextHardLimitTokens) {
        state.compactionRequested = true
        throw new Error(`Operational schema guard: emergency context ceiling reached ${state.lastInputTokens} tokens, indicating native compaction has not completed. Call no more tools, do not write a handoff, and end this turn concisely; OpenCode must compact and auto-continue this same session.`)
      }
      ...
      if (event?.type === "message.updated") {
        ...
        if (sessionID && info?.role === "assistant" && info?.finish) {
          ...
          if (Number.isFinite(inputTokens) && inputTokens >= 0) {
            ...
            if (inputTokens < policy.primaryContextWarningTokens) {
