...
const ASSESSMENT_EXIT = Object.freeze({ PASS: 0, FAIL: 1, BLOCKED: 2, STALE: 3, INFRA_ERROR: 4, ISOLATION_BREACH: 5 })
...
async function readCoreAuthority(stateDirectory, directory) {
  const value = await readJSON(coreStatePath(stateDirectory, directory), "workspace authority state", { missing: undefined })
...
function assessmentEvidence(output) {
  const text = String(output?.output ?? "")
  const marker = uniqueMatch(text, /^OPERATIONAL_ASSESSMENT: schema=([^;\n]+); assessment_id=([^;\n]+); spec_sha256=([0-9a-f]{64}); base_sha=([0-9a-f]{40}); target_sha=([0-9a-f]{40}); summary=([^\n]+)\s*$/gm)
...
function assessmentMatchesPending(evidence, pending, authority) {
  return Boolean(
    ...
    && evidence.schema === "opencode-local-assessment-v1"
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (assessment) {
        ...
        if (authority?.mode === "target" && authority.binding) {
          ...
          if (lifecycle?.target_sha === authority.binding) {
            throw new Error(`Operational schema guard: target ${authority.binding} already has an authenticated STALE assessment awaiting exact owner-base reconciliation; complete that reconciliation or declare a different exact target.`)
          }
      ...
      if (reconciliation) {
        ...
        if (!lifecycle || lifecycle.target_sha !== authority.binding) {
          throw new Error("Operational schema guard: owner-base reconciliation is not admitted by a generic target mismatch; the same exact repository-owned/base-authority assessment must first terminate with authenticated STALE evidence.")
        }
        ...
        if (
          ...
          throw new Error("Operational schema guard: owner-base reconciliation does not match the authenticated STALE assessment identity, spec bytes, owner SHA, pinned base, and persisted target.")
        }
      ...
      try {
        ...
        if (command && /reconcile-owner-base\.mjs/.test(String(error?.message ?? ""))) {
          ...
          if (authority?.mode === "target" && authority.binding && !lifecycle) {
            error.message = `${error.message} Owner-base reconciliation remains blocked until the same exact target assessment returns authenticated STALE evidence.`
          }
        }
        throw error
      }
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (assessment) {
        ...
        if (pending) {
          const authority = await readCoreAuthority(stateDirectory, directory)
          const evidence = assessmentEvidence(output)
          ...
          if (evidence.result === "STALE") {
            if (pending.reconcilable && SHA40.test(authority?.observedHead ?? "")) {
              await writeLifecycle(stateDirectory, directory, {
                ...
                owner_sha: authority.observedHead,
                spec_sha256: pending.specSha256,
                assessment_id: pending.assessmentID,
                spec_path: pending.specPath,
              })
              appendLifecycleNotice(output, `OPERATIONAL_TARGET_RECONCILIATION: admitted; target=${pending.targetSha}; base=${pending.baseSha}; owner=${authority.observedHead}; spec_sha256=${pending.specSha256}`)
            } else {
              await clearLifecycle(stateDirectory, directory)
              appendLifecycleNotice(output, `OPERATIONAL_TARGET_RECONCILIATION: not-admitted; STALE assessment is not repository-owned/base-authority or owner HEAD is unproven; target=${pending.targetSha}`)
            }
          } else {
            await clearLifecycle(stateDirectory, directory)
          }
      ...
      if (command) {
        ...
        if (authority?.mode === "target" && authority.binding && !lifecycle && /OPERATIONAL_AUTHORITY:\s*mismatch/.test(String(output?.output ?? ""))) {
          appendLifecycleNotice(output, "OPERATIONAL_TARGET_LIFECYCLE: owner-base reconciliation is not yet admitted; run the exact target assessment first and use the helper only after authenticated STALE evidence")
        }
      }
    },
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      const lifecycle = await readLifecycle(stateDirectory, directory).catch(() => undefined)
      if (lifecycle) {
        output.context.push(`Target lifecycle: OWNER_RECONCILIATION; target=${lifecycle.target_sha}; base=${lifecycle.base_sha}; owner=${lifecycle.owner_sha}; assessment_id=${lifecycle.assessment_id}; spec_sha256=${lifecycle.spec_sha256}.`)
      }
    },
