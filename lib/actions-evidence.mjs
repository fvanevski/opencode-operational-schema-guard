...
export async function fingerprintFiles(root, paths) {
  ...
  for (const relativePath of [...paths].sort()) {
    try {
      const bytes = await readFile(new URL(relativePath, `file://${root.endsWith("/") ? root : `${root}/`}`))
...
export function validateReceipt(receipt, profile, dispatch) {
  ...
  if (receipt.result === "BLOCKED") {
    if (typeof receipt.block_reason !== "string" || !/^[A-Z0-9_]{3,64}$/.test(receipt.block_reason)) throw new Error("BLOCKED receipt requires block_reason")
  } else if (receipt.block_reason !== null) throw new Error("non-BLOCKED receipt must have null block_reason")
