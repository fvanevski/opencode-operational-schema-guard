...
export const RUNNER_IMAGE_SCHEMA = "ghdev-runner-image-v3"
...
export function validateProfile(profile) {
  ...
  if (profile.runner.image_schema !== RUNNER_IMAGE_SCHEMA) throw new Error(`runner image schema must be ${RUNNER_IMAGE_SCHEMA}`)
  if (profile.runner.listener_mode !== "persistent-listener-v1") throw new Error("runner listener_mode must be persistent-listener-v1")
  if (profile.runner.runner_updates !== "disabled") throw new Error("runner runner_updates must be disabled")
  return profile
}
...
export function validateExecutionRecord(record, profile, dispatch) {
  ...
  if (executionStarted) {
    ...
    if (record.environment.listener_mode !== profile.runner.listener_mode) throw new Error("environment listener mode mismatch")
    if (record.environment.runner_updates !== profile.runner.runner_updates) throw new Error("environment runner update mode mismatch")
    for (const field of ["runner_settings_sha256", "runner_listener_sha256"]) if (typeof record.environment[field] !== "string" || !SHA256_RE.test(record.environment[field])) throw new Error(`${field} missing`)
    if (!Number.isSafeInteger(record.environment.runner_agent_id) || record.environment.runner_agent_id < 1) throw new Error("runner_agent_id missing")
    if (typeof record.environment.runner_name !== "string" || record.environment.runner_name.length < 1 || record.environment.runner_name.length > 256 || /[\r\n]/.test(record.environment.runner_name)) throw new Error("runner_name missing")
    if (typeof record.environment.runner_identity_clean_final !== "boolean") throw new Error("runner_identity_clean_final missing")
    ...
    for (const field of ["actions_runner_version", "git_version", "node_version", "npm_version", "python_version", "bwrap_version"]) {
    ...
    for (const field of ["os_release_fingerprint", "git_sha256", "node_sha256", "npm_sha256", "python_sha256", "bwrap_sha256"]) {
  ...
  if (record.result === "PASS") {
    ...
    if (record.environment.runner_identity_clean_final !== true) throw new Error("PASS requires unchanged live runner identity through final proof")
