...
const EXPLICIT_ROOT_TARGET_TOKEN = /(?:^|[^A-Za-z0-9_./\\-])([A-Za-z0-9_.-]+)(?!:\/\/)(?=$|[^A-Za-z0-9_./\\-])/gm
...
function extractExplicitTargetPaths(text, directory, { allowExistingRoot = false } = {}) {
  ...
  const paths = new Set(extractPaths(value, directory))
  for (const match of value.matchAll(EXPLICIT_ROOT_TARGET_TOKEN)) {
    const normalized = rootRelativeExplicitTarget(match[1], directory, { allowExisting: allowExistingRoot })
    if (normalized) paths.add(resolve(directory, normalized))
  }
  return paths
}
