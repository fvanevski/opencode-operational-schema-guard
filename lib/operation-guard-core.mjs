...
function shellInterpreterExecutionBodies(raw) {
  ...
  if (!new Set(["sh", "bash", "dash", "zsh", "ksh"]).has(executable)) return []
  ...
  for (const line of lines) {
    if (heredoc) {
      ...
      if (comparison === heredoc.delimiter) {
        if (heredoc.expandable) expandableBodies.push(heredoc.body.join("\n"))
        heredoc = undefined
    ...
    if (declaration) heredoc = { ...declaration, body: [] }
  }
  if (heredoc?.expandable) expandableBodies.push(heredoc.body.join("\n"))
