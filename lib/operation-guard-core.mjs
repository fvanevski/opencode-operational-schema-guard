...
export function normalizeTaskPacket(args, policy = DEFAULT_POLICY) {
  ...
  if (!envelope.scope && !envelope.stopCondition && envelope.questions.length === 0 && prompt.trim()) {
    ...
    const sectionOverflow = Boolean(sectionLimit && countNumberedSections(originalPrompt) > sectionLimit)
    if (!sectionOverflow && inferred.length + contractChars <= policy.taskPromptChars[type]) {
      prompt = inferred
...
function localAssessmentCommandError(command) {
  ...
  const stripped = stripRtk(raw)
  if (!stripped.startsWith(LOCAL_ASSESSMENT_RUNNER_PATH)) return undefined
  const normalized = normalizeExactWrapperContinuations(stripped)
  if (hasUnquotedShellOperator(normalized) || shellSegments(normalized).length !== 1) {
...
function localAssessmentInvocation(command) {
  const stripped = stripRtk(command)
  if (!stripped.startsWith(LOCAL_ASSESSMENT_RUNNER_PATH) || localAssessmentCommandError(command)) return undefined
  const normalized = normalizeExactWrapperContinuations(stripped)
...
function ownerBaseReconciliationInvocation(command) {
  const stripped = stripRtk(command)
  if (!stripped.startsWith(OWNER_BASE_RECONCILIATION_PATH) || ownerBaseReconciliationCommandError(command)) return undefined
  const normalized = normalizeExactWrapperContinuations(stripped)
  const parts = normalized.split(/\s+/)
