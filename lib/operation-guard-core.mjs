...
const FRESH_REVIEW_CONTEXT_PREFIX_SIGNAL = /^(?:Context|Supporting context):(?:\s|$)/i
const FRESH_REVIEW_CONTEXT_DIRECTIVE_SIGNAL = /(?:(?:^|[.!?;]\s+|\n\s*|\b(?:please|also|then|next|finally)\s+|\b(?:be\s+sure|make\s+sure|don't\s+forget|do\s+not\s+forget)\s+to\s+|\byou\s+(?:should|must|need\s+to|have\s+to)\s+)(?:review|inspect|check|examine|trace|follow|include|cover|consider|read|open|search(?:\s+for)?|scan|explore|investigate|look\s+(?:at|through|into|for)|expand|broaden|extend|widen|go\s+beyond)\b|\b(?:should|must|need\s+to|have\s+to)\s+(?:also\s+)?(?:be\s+)?(?:reviewed|inspected|checked|examined|traced|followed|included|covered|considered|read|opened|searched|scanned|explored|investigated)\b)/i
const FRESH_REVIEW_CONTEXT_SCOPE_SIGNAL = /\b(?:scope|in[- ]scope|out[- ]of[- ]scope|coverage|covers?|covered|covering|includes?|included|including|extends?|extended|extending|expands?|expanded|expanding|broadens?|broadened|broadening|widens?|widened|widening|beyond|outside|rest|remainder|surrounding|adjacent|neighboring|nearby|additional|other|related|relevant|dependencies|callers|modules|files|paths|targets|tests?)\b/i
const FRESH_REVIEW_CONTEXT_REQUIREMENT_SIGNAL = /\b(?:must|required|requires?|required|requiring|necessary|needed|should|need\s+to|have\s+to|ensure|verify|validate)\b/i
...
function freshReviewTrailingContextIsNonDirective(value) {
  const text = String(value ?? "").trim()
  if (!text) return true
  if (!FRESH_REVIEW_CONTEXT_PREFIX_SIGNAL.test(text)) return false
  const context = text.replace(/^(?:Context|Supporting context):\s*/i, "")
  return extractPaths(context, "/").size === 0
    && !FRESH_REVIEW_CONTEXT_DIRECTIVE_SIGNAL.test(context)
    && !FRESH_REVIEW_CONTEXT_SCOPE_SIGNAL.test(context)
    && !FRESH_REVIEW_CONTEXT_REQUIREMENT_SIGNAL.test(context)
}
...
function freshReviewEnvelopeInferenceIsBounded(prompt, policy) {
  ...
  if (!freshReviewTrailingContextIsNonDirective(trailingContext)) return false
...
export function normalizeTaskPacket(args, policy = DEFAULT_POLICY) {
  ...
  const inferenceScopeBounded = type === "fresh-review"
    ? freshReviewEnvelopeInferenceIsBounded(prompt, policy)
    : prompt.length <= 1200
