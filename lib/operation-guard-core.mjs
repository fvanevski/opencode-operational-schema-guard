...
function copyMoveTouchesTargets(command, workdir, roots, paths = [], { protectAncestors = false } = {}) {
  ...
  const touchesTarget = (target) => {
    ...
    const rootMatch = normalizedRoots.some((root) => isInsideDirectory(resolved, root) || (protectAncestors && isInsideDirectory(root, resolved)))
    const pathMatch = normalizedPaths.some((path) => path === resolved || (protectAncestors && isInsideDirectory(path, resolved)))
    return rootMatch || pathMatch
  }
  for (const raw of shellSegments(command)) {
...
function invocationTouchesProtectedTarget(tool, args, directory, protectedRoots, protectedPaths) {
  ...
  const touchesTarget = (path) => {
    ...
    return roots.some((root) => isInsideDirectory(resolved, root) || isInsideDirectory(root, resolved))
      || paths.some((protectedPath) => protectedPath === resolved || isInsideDirectory(protectedPath, resolved))
  }
  ...
  if (roots.some((root) => isInsideDirectory(workdir, root)) || copyMoveTouchesTargets(command, workdir, roots, paths, { protectAncestors: true })) return true
