...
function authorityTargetingSetup(command, binding, workdir, directory) {
  ...
  if (target.test(segment)) return { kind: "current", proofWorkdir: resolve(directory) }
  const worktree = segment.match(new RegExp(`^git\\s+worktree\\s+add\\s+(?:--detach\\s+)?([^\\s;&|<>]+)\\s+${escaped}$`))
  return worktree ? { kind: "worktree", proofWorkdir: resolve(workdir, worktree[1]) } : undefined
}
