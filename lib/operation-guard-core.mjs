...
function copyLikeOperands(tokens, { targetDirectory = false, shortValueOptions = "", longValueOptions = [] } = {}) {
  const positional = []
  ...
  for (let index = 1; index < tokens.length; index += 1) {
    ...
    if (options && token.startsWith("--")) {
      ...
      if (!token.includes("=") && longValues.has(name)) {
    ...
    if (options && token.startsWith("-") && token !== "-") {
      ...
      const valueIndex = [...cluster].findIndex((character) => shortValueOptions.includes(character))
      if (valueIndex >= 0) {
  ...
  return { ambiguous: false, positional, destinationDirectory, directoryMode, removeSourceFiles }
}
...
function mutationSegmentWorkspaceOwnership(raw, workdir, directory) {
  ...
  if (new Set(["cp", "ln", "mv", "install", "rsync"]).has(verb)) {
    recognized = true
    const options = verb === "cp" || verb === "mv"
      ? { targetDirectory: true, shortValueOptions: "S", longValueOptions: ["suffix"] }
      : verb === "ln"
        ? { targetDirectory: true, shortValueOptions: "S", longValueOptions: ["suffix"] }
        : verb === "install"
          ? { targetDirectory: true, shortValueOptions: "gmoS", longValueOptions: ["group", "mode", "owner", "suffix", "strip-program"] }
          : { targetDirectory: false, shortValueOptions: "e", longValueOptions: ["exclude", "include", "filter", "files-from", "exclude-from", "include-from", "rsync-path", "rsh"] }
    ...
    const parsed = copyLikeOperands(tokens, { shortValueOptions: valueOptions, longValueOptions: verb === "mkdir" ? ["mode"] : verb === "touch" ? ["date", "reference", "time"] : verb === "truncate" ? ["reference", "size"] : [] })
    ambiguous ||= parsed.ambiguous
    if (!parsed.ambiguous) {
      if (parsed.positional.length < 1) ambiguous = true
      else parsed.positional.forEach(add)
    ...
    recognized = true
    const parsed = copyLikeOperands(tokens, { longValueOptions: ["reference", ...(verb === "chown" ? ["from"] : [])] })
    ambiguous ||= parsed.ambiguous
    if (!parsed.ambiguous) {
      const paths = parsed.positional.slice(1)
