...
const EXPLICIT_ROOT_TARGET_LEADING_TOKEN = /^[`'"]?([A-Za-z0-9_.-]+)[`'"]?(?=$|[\s,;:!?])/
const EXPLICIT_ROOT_TARGET_FOLLOWUP_TOKEN = /(?:\s+(?:and|&)\s+|,\s*)[`'"]?([A-Za-z0-9_.-]+)[`'"]?(?=$|[\s,;:!?])/g
...
function rootRelativeExplicitTarget(value, directory, { allowExisting = false } = {}) {
  ...
  try {
    ...
    return info.isFile() || info.isDirectory() ? candidate : undefined
  } catch {
    return undefined
  }
}
...
function extractExplicitTargetPaths(text, directory, { allowExistingRoot = false } = {}) {
  const paths = new Set(extractPaths(String(text), directory))
  const value = String(text ?? "").trim()
  const candidates = []
  const leading = value.match(EXPLICIT_ROOT_TARGET_LEADING_TOKEN)
  if (leading) candidates.push(leading[1])
  for (const match of value.matchAll(EXPLICIT_ROOT_TARGET_FOLLOWUP_TOKEN)) candidates.push(match[1])
  for (const candidate of candidates) {
    const normalized = rootRelativeExplicitTarget(candidate, directory, { allowExisting: allowExistingRoot })
...
export function validateTaskPacket(args, policy = DEFAULT_POLICY, { allowKnownExactExplore = false } = {}) {
  ...
  const targets = explicitTargets.length > 0
    ? new Set(explicitTargets.flatMap((target, index) => {
        ...
        const paths = [...extractExplicitTargetPaths(target, "/")]
        if (type === "explore") return paths.length > 0 ? paths : [`target:${index}`]
        return [paths[0] ?? `target:${index}`]
      }))
    : extractPaths(prompt, "/")
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (tool === "task") {
        ...
        try {
          ...
          validateTaskPacket(output.args, policy, { allowKnownExactExplore: plannerPartitionReplay })
