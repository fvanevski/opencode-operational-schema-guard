...
function copyMoveTouchesRoots(command, workdir, roots) {
  const normalizedRoots = roots.map((root) => resolve(root))
  const touchesRoot = (target) => normalizedRoots.some((root) => isInsideDirectory(resolve(workdir, target), root))
  for (const raw of shellSegments(command)) {
    ...
    for (let index = 1; index < tokens.length; index += 1) {
      ...
      if (options && token === "--target-directory") {
        ...
        if (target && touchesRoot(target)) return true
        index += 1
        continue
      }
      ...
      if (touchesRoot(token)) return true
    }
  }
  return false
}

function invocationTouchesProtectedRoot(tool, args, directory, protectedRoots) {
  if (protectedRoots.length === 0) return false
  const roots = protectedRoots.map((root) => resolve(root))
  const touchesRoot = (path) => roots.some((root) => isInsideDirectory(path, root))
  if (EDIT_TOOLS.has(tool)) {
    const path = editPath(args, directory)
    return Boolean(path && touchesRoot(path))
  }
  if (tool !== "bash" && tool !== "shell") return false
  const workdir = args?.workdir ? resolve(directory, args.workdir) : resolve(directory)
  const command = String(args?.command ?? "")
  if (touchesRoot(workdir) || copyMoveTouchesRoots(command, workdir, roots)) return true
  if ([...extractPaths(command, workdir)].some(touchesRoot)) return true
...
function invocationOwnedByWorkspace(tool, args, directory) {
  ...
  const workdir = args?.workdir ? resolve(directory, args.workdir) : resolve(directory)
  const command = String(args?.command ?? "")
  if (copyMoveTouchesRoots(command, workdir, [directory])) return true
...
export function createOperationGuard({ directory = process.cwd(), env = process.env, policy = policyFromEnv(env), stateDirectory, client, protectedMutationRoots = [] } = {}) {
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (invocationTouchesProtectedRoot(tool, output.args, directory, protectedRoots)) {
