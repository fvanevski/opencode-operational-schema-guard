...
function emptySafetyState() {
  return {
    ...
    targetProofRepositories: new Map(),
...
function beginAuthorityAdmission(state, declaration) {
  ...
  if (authorityEpochChanged) {
    ...
    state.targetProofRepositories.clear()
  }
...
function authorityTargetingSetup(command, binding, workdir, directory) {
  ...
  const governedDirectory = resolve(directory)
  const escaped = binding.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
  const segments = shellSegments(command)
  ...
  if (target.test(segment)) return resolvedWorkdir === governedDirectory ? { kind: "current", proofWorkdir: governedDirectory } : undefined
  const worktree = segment.match(new RegExp(`^git\\s+worktree\\s+add\\s+(?:--detach\\s+)?([^\\s;&|<>]+)\\s+${escaped}$`))
  if (!worktree) return undefined
  const repositoryCommonDir = gitMetadataPath(resolvedWorkdir, "--git-common-dir")
  if (!repositoryCommonDir || !gitCommitExists(resolvedWorkdir, binding)) return undefined
  const governedRepositoryCommonDir = gitMetadataPath(governedDirectory, "--git-common-dir")
  if (
    governedRepositoryCommonDir
    && gitCommitExists(governedDirectory, binding)
    && repositoryCommonDir !== governedRepositoryCommonDir
  ) return undefined
  return { kind: "worktree", proofWorkdir: resolve(resolvedWorkdir, worktree[1]), repositoryCommonDir }
}

function gitCommitExists(workdir, binding) {
  const result = spawnSync("git", ["-C", resolve(workdir), "cat-file", "-e", `${binding}^{commit}`], {
    encoding: "utf8",
    timeout: 5_000,
    maxBuffer: 64 * 1024,
...
function targetProofWorkdirMatchesRepositoryCommon(workdir, repositoryCommonDir) {
  const resolvedWorkdir = resolve(workdir)
  ...
  if (!repositoryCommonDir || !targetCommon || !targetGitDir || !targetTop) return false
  const worktreesRoot = join(repositoryCommonDir, "worktrees")
  return targetCommon === repositoryCommonDir
...
function authorityProofWorkdirAllowed(state, workdir, directory) {
  ...
  const repositoryCommonDir = state.targetProofRepositories.get(resolved)
  if (repositoryCommonDir && targetProofWorkdirMatchesRepositoryCommon(resolved, repositoryCommonDir)) return true
  state.targetProofWorkdirs.delete(resolved)
  state.targetProofRepositories.delete(resolved)
  return false
}
...
function authorityProofAdmissionCurrent(state, admission, directory) {
  ...
  const resolvedWorkdir = resolve(admission.workdir)
  if (admission.mode === "target" && resolvedWorkdir !== resolve(directory)) {
    const repositoryCommonDir = state.targetProofRepositories.get(resolvedWorkdir)
    if (!repositoryCommonDir || !targetProofWorkdirMatchesRepositoryCommon(resolvedWorkdir, repositoryCommonDir)) {
      state.targetProofWorkdirs.delete(resolvedWorkdir)
      state.targetProofRepositories.delete(resolvedWorkdir)
      return false
    }
  }
      ...
      if (authorityPending && authorityCommand) {
        ...
        if (targetSetup) {
          state.targetSetupAdmissions.set(input.callID, {
            ...
            repositoryCommonDir: targetSetup.repositoryCommonDir,
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (PRIMARY_AGENTS.has(state.agent) && (tool === "bash" || tool === "shell")) {
        ...
        if (
          ...
          && targetProofWorkdirMatchesRepositoryCommon(targetSetupAdmission.proofWorkdir, targetSetupAdmission.repositoryCommonDir)
        ) {
          state.targetProofWorkdirs.add(targetSetupAdmission.proofWorkdir)
          state.targetProofRepositories.set(targetSetupAdmission.proofWorkdir, targetSetupAdmission.repositoryCommonDir)
          await safetyStore.save()
        }
