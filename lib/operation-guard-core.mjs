...
function copyMoveTouchesTargets(command, workdir, roots, paths = []) {
  ...
  const touchesTarget = (target) => {
    ...
    return normalizedRoots.some((root) => isInsideDirectory(resolved, root)) || normalizedPaths.includes(resolved)
  }
  for (const raw of shellSegments(command)) {
...
function destructiveAncestorTouchesProtected(command, workdir, roots, paths) {
  const containsProtected = (target) => {
    const resolved = resolve(workdir, target)
    return roots.some((root) => isInsideDirectory(root, resolved)) || paths.some((path) => isInsideDirectory(path, resolved))
  }
  for (const raw of shellSegments(command)) {
    const tokens = shellWordTokens(canonicalValidationSegment(raw))
...
function invocationTouchesProtectedTarget(tool, args, directory, protectedRoots, protectedPaths) {
  ...
  const touchesTarget = (path) => {
    ...
    return roots.some((root) => isInsideDirectory(resolved, root)) || paths.includes(resolved)
  }
...
function redirectionWriteTargets(command, workdir) {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    const match = remainder.match(/^(?:"([^"]+)"|'([^']+)'|([^\s;&|]+))/)
    const target = match?.[1] ?? match?.[2] ?? match?.[3]
    const resolved = staticShellPath(target, workdir)
    if (!target || !resolved) ambiguous = true
