...
const FRESH_REVIEW_BOUNDED_SCOPE_SIGNAL = /\b(?:bounded|diff-relevant|explicitly\s+(?:named|listed|specified)|specific(?:ally)?\s+(?:named|listed|specified)|(?:current|this|implementation|provided|attached|changed)\s+(?:pull request|PR|diff|change(?:\s+set)?|changes|files?|symbols?)|exact(?:ly)?\s+(?:the\s+)?(?:named|listed|specified|current|provided|implementation))\b/i
const FRESH_REVIEW_OPEN_ENDED_SCOPE_SIGNAL = /\b(?:(?:entire|whole)\s+(?:repo(?:sitory)?|codebase|project)|(?:repo(?:sitory)?|codebase|project)[ -]wide|follow\s+every|every\s+(?:caller|dependency|file|path|module|symbol|reference)|all\s+(?:callers|dependencies|files|paths|modules|symbols|references)|(?:anything|everything|whatever)(?:\s+else)?(?:\s+(?:necessary|required|needed|relevant))?)\b/i
...
function planCommandLines(text) {
  const commandStart = /^(?:(?:[-*]|\d+[.)])\s*)?(?:`{1,3})?(?:rtk\s+|env\s+|[A-Za-z_][A-Za-z0-9_]*=\S+\s+)*(?:(?:\/[^\s`]+\/|\.venv[^\s`]*\/bin\/)(?:verify-disposable\.mjs|verify-manifest\.mjs|local-agent-assessment\.mjs|workspace-identity\.mjs|session-trace-assessment\.mjs|python(?:3(?:\.\d+)?)?|pytest|ruff|pyrefly|mypy)\b|scripts\/disposable-test-services\b|git\s|node\s|python(?:3(?:\.\d+)?)?\s|pytest\s|ruff\s|pyrefly\s|mypy\s|npm\s|pnpm\s|yarn\s|cargo\s|go\s|ls(?:\s|$)|grep\s|rg\s)/i
  const commands = []
