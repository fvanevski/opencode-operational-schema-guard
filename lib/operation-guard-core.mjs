  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (state.authorityBinding && authorityCommand) {
        ...
        if (state.authorityMode === "target" && state.authorityStatus === "verified" && boundTargetingSetup(authorityCommand, state.authorityBinding)) {
          ...
          if (!authorityTargetingSetup(authorityCommand, state.authorityBinding, authorityWorkdir, directory)) {
            throw await correctionError(`Operational schema guard: OPERATIONAL_CORRECTION: ADMIT_EXACT_TARGET; mode=target; status=verified; required=${state.authorityBinding}; offending_workdir=${JSON.stringify(authorityWorkdir)}; governed_workdir=${JSON.stringify(resolve(directory))}; do_not_execute=true. Bound-target setup remains owned by the governed session repository after verification. Run the standalone setup from the governed repository; admitted disposable target worktrees remain proof workdirs and do not become alternate setup authority.`)
          }
        }
      }
