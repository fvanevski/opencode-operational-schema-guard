...
import { actionsReceiptLedgerEntry, CHILD_PLAN_SCHEMA, parseChildTerminal, planChildWork, semanticReviewAuthorityFromMessage, semanticReviewRequirement } from "./hybrid-workflow.mjs"
...
function preservePlannerPartitionContext(prompt, planning, directory) {
...
async function deterministicTaskPlan(args, state, directory, policy) {
  ...
  const envelope = parsePacketEnvelope(String(args?.prompt ?? ""))
  ...
  const authorityHead = /^[0-9a-f]{40}$/.test(state.taskWorkspaceHead ?? "") && state.taskWorkspaceHeadStatus === "proven"
    ...
    : /^[0-9a-f]{40}$/.test(state.authorityBinding ?? "")
      ? state.authorityBinding
      : undefined
  if (!authorityHead) return undefined

  const actionsEvidence = type === "verify" ? await trustedActionsPlanEvidence(envelope, state, authorityHead) : undefined
  const planningBaseSha = state.planningBaseSha ?? state.campaignBaseHead
  const targets = []
  for (const target of envelope.targets) {
    ...
    const metrics = targetDiffMetrics(directory, planningBaseSha, repoRelative, fileBytes, state.editedPaths.has(absolute))
    targets.push({
      path: repoRelative,
  ...
  const planning = planChildWork({
    ...
    role: type,
    authority: { repository: actionsEvidence?.entry.repository ?? "local/workspace", head_sha: authorityHead, generation: state.editGeneration },
  ...
  return preservePlannerPartitionContext(String(args?.prompt ?? ""), planning, directory)
}
