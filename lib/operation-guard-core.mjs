...
function freshReviewEnvelopeInferenceIsBounded(prompt, policy) {
  ...
  if (
    FRESH_REVIEW_OPEN_ENDED_SCOPE_SIGNAL.test(text)
    || FRESH_REVIEW_SCOPE_EXPANSION_SIGNAL.test(text)
    || FRESH_REVIEW_META_SCOPE_EXPANSION_SIGNAL.test(text)
    || FRESH_REVIEW_INVESTIGATIVE_UNBOUNDED_SIGNAL.test(text)
    || FRESH_REVIEW_SCOPE_CONTRADICTION_SIGNAL.test(text)
  ) return false

  const lines = text.split(/\r?\n/)
  const firstDirective = lines.find((line) => line.trim())?.trim()
  ...
  if (FRESH_REVIEW_EXPLICIT_DIFF_BOUNDARY_SIGNAL.test(firstDirective)) return true

  const directiveTargets = extractPaths(firstDirective, "/")
  return targets.size > 0
    && directiveTargets.size === targets.size
    && FRESH_REVIEW_EXPLICIT_TARGET_BOUNDARY_SIGNAL.test(firstDirective)
}
