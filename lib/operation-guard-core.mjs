...
function containsHeadProofAttempt(command) {
  const text = String(command)
  if (/\bgit\s+rev-parse\s+HEAD\b/i.test(text)) return true
  return shellSegments(text).some((segment) => /^git\s+rev-parse\s+HEAD(?:\s|$)/i.test(canonicalGitSegment(segment)))
}
