...
export function validateTaskPacket(args, policy = DEFAULT_POLICY, { allowKnownExactExplore = false } = {}) {
  ...
  const targets = explicitTargets.length > 0
    ? new Set(explicitTargets.flatMap((target, index) => {
        ...
        const paths = [...extractPaths(target, "/")]
...
function partitionPacketFromOriginalPrompt(prompt, selectedPaths, directory) {
  ...
  for (const line of lines.slice(start, end)) {
    ...
    if (!match) return undefined
    const paths = [...extractPaths(match[1], directory)].filter((path) => isInsideDirectory(path, directory))
...
async function deterministicTaskPlan(args, state, directory, policy) {
  ...
  for (const target of envelope.targets) {
    ...
    const paths = [...extractPaths(target, directory)].filter((path) => isInsideDirectory(path, directory))
