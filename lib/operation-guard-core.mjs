...
function freshReviewDescriptionIsBounded(description) {
  const value = String(description ?? "").trim()
  return Boolean(value && !/[\r\n]/.test(value) && FRESH_REVIEW_BOUNDED_DESCRIPTION_SIGNAL.test(value))
}

function freshReviewEnvelopeInferenceIsBounded(prompt, policy) {
  const text = String(prompt)
  const lines = text.split(/\r?\n/)
  const firstDirectiveIndex = lines.findIndex((line) => line.trim())
  if (firstDirectiveIndex < 0) return false
  const firstDirective = lines[firstDirectiveIndex].trim()
  const trailingContext = lines.slice(firstDirectiveIndex + 1).join("\n").trim()
  if (trailingContext) return false
  ...
  if (targets.size > policy.taskExplicitTargets["fresh-review"]) return false

  const diffBoundary = firstDirective.match(FRESH_REVIEW_EXPLICIT_DIFF_BOUNDARY_SIGNAL)
  if (diffBoundary) {
    return FRESH_REVIEW_BENIGN_RESULT_SUFFIX_SIGNAL.test(firstDirective.slice(diffBoundary[0].length))
  }

  const spans = freshReviewDirectivePathSpans(firstDirective)
  if (spans.length === 0 || targets.size === 0) return false
  const directiveTargets = new Set(spans.map((span) => span.path))
  if (directiveTargets.size !== targets.size) return false
  if (!FRESH_REVIEW_EXPLICIT_TARGET_PREFIX_SIGNAL.test(firstDirective.slice(0, spans[0].start))) return false
  for (let index = 1; index < spans.length; index += 1) {
    if (!FRESH_REVIEW_TARGET_SEPARATOR_SIGNAL.test(firstDirective.slice(spans[index - 1].end, spans[index].start))) return false
  }
  return FRESH_REVIEW_BENIGN_RESULT_SUFFIX_SIGNAL.test(firstDirective.slice(spans.at(-1).end))
}
...
export function normalizeTaskPacket(args, policy = DEFAULT_POLICY) {
  ...
  const inferenceScopeBounded = type === "fresh-review"
    ? freshReviewEnvelopeInferenceIsBounded(prompt, policy) && freshReviewDescriptionIsBounded(description)
    : prompt.length <= 1200
    ...
    "chat.message": async (input, output) => {
      ...
      if (PRIMARY_AGENTS.has(state.agent)) {
        ...
        if (declaration) {
          ...
          if (previousBinding && previousBinding !== declaration.binding) {
