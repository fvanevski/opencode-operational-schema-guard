...
function stripShellExecutionPrefixes(raw) {
  ...
  do {
    ...
    segment = segment.replace(/^time(?:\s+-p)?\s+/i, "")
    segment = segment.replace(/^(?:command|exec)\s+(?:--\s+)?/i, "")
  } while (segment !== previous)
  return segment
}

function shellInterpreterExecutionBodies(raw) {
  const segment = stripShellExecutionPrefixes(raw)
  const tokens = shellWordTokens(canonicalValidationSegment(segment))
...
function executableHeadProofSegment(raw) {
  const segment = stripShellExecutionPrefixes(raw)
  return /^git\s+rev-parse\s+HEAD(?:\s|$)/i.test(canonicalGitSegment(segment))
}
