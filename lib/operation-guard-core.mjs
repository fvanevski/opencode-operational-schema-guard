  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (tool === "task") {
        ...
        const plannerPartitionReplay = state.admittedPlannerPacketHashes.has(plannerReplayKey)
        let planning
        try {
          ...
          if (planning?.status === "PARTITION_REQUIRED") {
            for (const partition of planning.partitions) {
              state.admittedPlannerPacketHashes.add(`${requestedType}:${normalizedPlannerPacketHash(requestedType, partition.packet, policy)}`)
            }
          }
          ...
          if (plannerPartitionReplay) state.admittedPlannerPacketHashes.delete(plannerReplayKey)
        } catch (error) {
