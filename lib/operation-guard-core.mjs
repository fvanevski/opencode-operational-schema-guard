...
function staticShellPath(value, workdir) {
  ...
  if (!token || token === "-" || /[*?\[\]{}$`()<>]/.test(token) || /^~(?:\/|$)/.test(token)) return undefined
...
function copyLikeOperands(tokens, { targetDirectory = false, shortFlagOptions = "", shortValueOptions = "", longFlagOptions = [], longValueOptions = [] } = {}) {
  ...
  return { ambiguous: false, positional, destinationDirectory, directoryMode, removeSourceFiles, referenceMode, symbolicMode, hardLinkMode }
}
...
function mutationSegmentWorkspaceOwnership(raw, workdir, directory) {
  ...
  if (new Set(["cp", "ln", "mv", "install", "rsync"]).has(verb)) {
    ...
    if (!parsed.ambiguous) {
      ...
      } else if (parsed.destinationDirectory !== undefined) {
        add(parsed.destinationDirectory)
        const sourcesWrite = verb === "mv" || parsed.removeSourceFiles || (verb === "ln" && !parsed.symbolicMode) || (verb === "cp" && parsed.hardLinkMode)
        if (sourcesWrite) parsed.positional.forEach(add)
      } else if (parsed.positional.length < 2) {
        ambiguous = true
      } else if (verb === "mv") {
        parsed.positional.forEach(add)
      } else {
        add(parsed.positional.at(-1))
        const sourcesWrite = parsed.removeSourceFiles || (verb === "ln" && !parsed.symbolicMode) || (verb === "cp" && parsed.hardLinkMode)
        if (sourcesWrite) parsed.positional.slice(0, -1).forEach(add)
      }
    }
