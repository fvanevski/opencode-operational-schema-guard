...
function gitMutationWorkdir(tokens, workdir) {
  ...
  for (let index = 1; index < tokens.length; index += 1) {
    ...
    if (token === "--git-dir" || token === "--work-tree") {
      ...
      const resolved = staticShellPath(target, current)
      if (!resolved) return undefined
      if (token === "--work-tree") current = resolved
      index += 1
      continue
    }
...
function mutationSegmentWorkspaceOwnership(raw, workdir, directory) {
  ...
  } else if (verb === "git" && /^(?:git\s+)(?:(?:-\S+(?:\s+\S+)?\s+)*)(?:apply|merge|rebase|cherry-pick|restore|reset|clean|checkout)\b/.test(segment)) {
    recognized = true
    const target = gitMutationWorkdir(tokens, workdir)
    if (!target) ambiguous = true
...
function invocationOwnedByWorkspace(tool, args, directory) {
  ...
  const workdir = args?.workdir ? resolve(directory, args.workdir) : resolve(directory)
  const command = String(args?.command ?? "")
  if (workspaceMutationReason(command)) {
  ...
  return {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (authorityPending && authorityCommand) {
        ...
        const mutation = authorityMutation || Boolean(workspaceMutationReason(authorityCommand)) || containsPublishCommand(authorityCommand)
