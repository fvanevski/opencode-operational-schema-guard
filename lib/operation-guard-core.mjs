...
export function createOperationGuard({ directory = process.cwd(), env = process.env, policy = policyFromEnv(env), stateDirectory, client, protectedMutationRoots = [], protectedMutationPaths = [] } = {}) {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      if (PRIMARY_AGENTS.has(state.agent)) {
        ...
        if (declaration) {
          const previousBinding = state.authorityBinding
          const previousMode = state.authorityMode
          beginAuthorityAdmission(state, declaration)
          if (previousBinding !== state.authorityBinding || previousMode !== state.authorityMode) {
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (authorityPending && EDIT_TOOLS.has(tool) && invocationOwnedByWorkspace(tool, output.args, directory)) {
        ...
        if (!externalTemporaryPath) {
          ...
          throw new Error(`Operational schema guard: exact-head admission is ${state.authorityStatus} for ${state.authorityBinding}. ${guidance}`)
        }
      }
      if (authorityPending && authorityCommand) {
        ...
        if (proofShapeCorrection) throw new Error(`Operational schema guard: ${proofShapeCorrection}`)
        if (exactHeadProofOnly(authorityCommand)) {
          if (!authorityProofWorkdirAllowed(state, authorityWorkdir, directory)) {
            ...
            throw new Error(`Operational schema guard: ${correction}`)
          }
        ...
        if (mutation && (authorityMutation || invocationOwnedByWorkspace(tool, output.args, directory)) && !allowedTarget) {
          const guidance = state.authorityStatus === "pending"
            ...
            : state.authorityMode === "strict-start"
              ? "Strict-start mismatch requires new user starting-revision authority; no conversational bypass exists."
              : targetRecoveryGuidance(state.authorityBinding)
          throw new Error(`Operational schema guard: exact-head admission is ${state.authorityStatus} (${state.authorityMode}) for ${state.authorityBinding}. ${guidance}`)
        }
      }
