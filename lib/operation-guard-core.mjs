...
function mutationSegmentWorkspaceOwnership(raw, workdir, directory) {
  ...
  if (new Set(["cp", "ln", "mv", "install", "rsync"]).has(verb)) {
    ...
    const options = verb === "cp"
      ...
      : verb === "mv"
        ? { targetDirectory: true, shortFlagOptions: "bfinTuv", shortValueOptions: "S", longFlagOptions: ["backup", "debug", "exchange", "force", "interactive", "no-clobber", "no-copy", "no-target-directory", "strip-trailing-slashes", "update", "verbose"], longValueOptions: ["suffix"] }
        : verb === "ln"
    ...
    recognized = true
    const paths = [...extractPaths(segment, workdir)]
    if (paths.length === 0) targets.push(resolve(workdir))
    else targets.push(...paths)
