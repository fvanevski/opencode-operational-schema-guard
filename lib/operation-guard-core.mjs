...
function relativeCopyMoveTouchesWorkspace(command, directory) {
  for (const raw of shellSegments(command)) {
    ...
    for (const token of tokens.slice(1)) {
      ...
      if (isInsideDirectory(resolve(directory, token), directory)) return true
    }
  }
  return false
}
...
function invocationOwnedByWorkspace(tool, args, directory) {
  ...
  if (relativeCopyMoveTouchesWorkspace(command, workdir)) return true
