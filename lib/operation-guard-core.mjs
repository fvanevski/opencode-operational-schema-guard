...
function beginAuthorityAdmission(state, declaration) {
  ...
  if (!changed && state.authorityMode === "target" && declaration.mode === "strict-start") {
  ...
  if (changed && authorityScopedStateNeedsReset(state)) resetForAuthorityChange(state, state.authorityBinding ?? "released/unbound", declaration.binding)
    ...
    "chat.message": async (input, output) => {
      ...
      if (PRIMARY_AGENTS.has(state.agent)) {
        ...
        if (declaration) {
          ...
          if (previousBinding !== declaration.binding) {
