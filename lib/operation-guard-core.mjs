...
function beginAuthorityAdmission(state, declaration) {
  ...
  if ((changed || modeChanged) && authorityScopedStateNeedsReset(state)) {
    const previous = changed ? (state.authorityBinding ?? "released/unbound") : `${state.authorityBinding} (${previousMode ?? "none"})`
    const next = changed ? declaration.binding : `${declaration.binding} (${declaration.mode})`
