...
function localAssessmentCommandError(command) {
  ...
  if (hasUnquotedShellOperator(normalized) || shellSegments(normalized).length !== 1) {
    return `Local assessment must be one logical bare invocation with no shell operators. It may use one physical line or unquoted backslash-LF/CRLF continuations only when they canonicalize to that exact invocation.`
  }
...
function ownerBaseReconciliationCommandError(command) {
  ...
  const normalized = normalizeExactWrapperContinuations(stripped)
  if (hasUnquotedShellOperator(normalized) || shellSegments(normalized).length !== 1) {
    return `Owner-base reconciliation must be one logical bare invocation with no shell operators. It may use one physical line or unquoted backslash-LF/CRLF continuations only when they canonicalize to that exact invocation.`
  }
  const parts = normalized.split(/\s+/)
  if (
    parts.length !== 9
