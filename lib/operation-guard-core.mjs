...
function copyLikeOperands(tokens, { targetDirectory = false, shortFlagOptions = "", shortValueOptions = "", longFlagOptions = [], longValueOptions = [] } = {}) {
  ...
  return { ambiguous: false, positional, destinationDirectory, directoryMode, removeSourceFiles, referenceMode }
}
  ...
  if (new Set(["cp", "ln", "mv", "install", "rsync"]).has(verb)) {
    ...
    if (!parsed.ambiguous) {
      if (verb === "install" && parsed.directoryMode) {
        ...
        if (verb === "mv" || parsed.removeSourceFiles) parsed.positional.forEach(add)
        ...
        if (parsed.removeSourceFiles) parsed.positional.slice(0, -1).forEach(add)
      }
    }
...
function invocationOwnedByWorkspace(tool, args, directory) {
  ...
  const workdir = args?.workdir ? resolve(directory, args.workdir) : resolve(directory)
  const command = String(args?.command ?? "")
  if (workspaceMutationReason(command, workdir, directory)) {
