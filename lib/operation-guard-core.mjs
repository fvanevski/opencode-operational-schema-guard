      ...
      if (PRIMARY_AGENTS.has(state.agent) && (tool === "bash" || tool === "shell") && Number(output?.metadata?.exit) === 0) {
        ...
        if (proof) {
          ...
          if (!authorityProofAdmissionCurrent(state, proofAdmission, directory)) {
            ...
            output.output = `${String(output.output ?? "")}${String(output.output ?? "").endsWith("\n") ? "" : "\n"}OPERATIONAL_AUTHORITY_PROOF: STALE; admitted_epoch=${proofAdmission.epoch}; current_epoch=${state.authorityEpoch}; admitted_mode=${proofAdmission.mode}; current_mode=${state.authorityMode ?? "none"}; current_status=${state.authorityStatus ?? "none"}. The completion cannot change authority or workspace provenance.`
          } else {
