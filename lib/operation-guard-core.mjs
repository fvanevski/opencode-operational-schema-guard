...
function shellControlExecutionSegments(command) {
  ...
  for (let index = 0; index < text.length; index += 1) {
    ...
    const groupingOpen = char === "(" && (index === 0 || /[\s;|&{}()]/.test(previous))
    if (char === "\n" || char === ";" || char === "|" || char === "&" || char === "{" || char === "}" || char === ")" || groupingOpen) {
      flush()
...
function containsHeadProofAttempt(command) {
  ...
  return segments.some((segment) => shellInterpreterExecutionBodies(segment).some((body) => containsHeadProofAttempt(body)))
}
