...
function partitionPacketFromOriginalPrompt(prompt, selectedPaths, directory) {
  const lines = String(prompt).split(/\r?\n/)
  const headings = lines.map((line, index) => /^Targets:\s*$/i.test(line.trim()) ? index : -1).filter((index) => index >= 0)
  if (headings.length !== 1) return undefined
  const start = headings[0] + 1
  let end = lines.length
  for (let index = start; index < lines.length; index += 1) {
    if (/^[A-Za-z][A-Za-z -]+:\s*/.test(lines[index].trim())) {
      end = index
      break
    }
  }
  ...
  return [...lines.slice(0, start), ...selected, ...lines.slice(end)].join("\n")
}
