...
const AUTHORITY_LABEL = /(?:^|[\r\n])[ \t]*(HEAD_SHA|EXPECTED_HEAD_SHA|EXPECTED_START_HEAD(?:_SHA)?|REQUIRED_START_HEAD_SHA|AUTHORITATIVE_HEAD_SHA|FINAL_HEAD_SHA|PR_HEAD_SHA|CANDIDATE_SHA|REQUIRED[ \t]+STARTING[ \t]+HEAD(?:[ \t]+SHA)?|EXPECTED[ \t]+STARTING[ \t]+HEAD(?:[ \t]+SHA)?|REQUIRED(?:[ \t]+EXACT)?[ \t]+HEAD|EXPECTED(?:[ \t]+BRANCH)?[ \t]+HEAD|REQUIRED[ \t]+PR[ \t]+HEAD)(?:[ \t]*(?:=|:)[ \t]*|[ \t]+)[`'"]*[ \t]*([0-9a-f]{40})\b[ \t]*[`'"),.;:!?\]}]*[ \t]*(?=$|[\r\n])/gi
