...
const FRESH_REVIEW_SCOPE_EXPANSION_SIGNAL = /\b(?:(?:any|other|additional|related|relevant)\s+(?:callers?|dependencies|files?|paths?|modules?|symbols?|references?|code|logic)|(?:callers?|dependencies|files?|paths?|modules?|symbols?|references?|code|logic)\s+(?:as|if)\s+(?:needed|necessary|required|relevant)|(?:anything|everything|whatever)\s+(?:callers?|dependencies|files?|paths?|modules?|symbols?|references?|code|logic)\b[^.\n]{0,32}\b(?:needed|necessary|required|relevant))\b/i
