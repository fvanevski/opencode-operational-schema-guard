...
function preservePlannerPartitionContext(prompt, planning, directory, policy, targets) {
  if (!planning || !new Set(["READY", "PARTITION_REQUIRED"]).has(planning.status) || targets.length === 0) return planning
...
async function deterministicTaskPlan(args, state, directory, policy) {
  ...
  return preservePlannerPartitionContext(prompt, planning, directory, policy, targets)
}
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (tool === "task") {
        ...
        state.pendingTasks.set(input.callID, {
          ...
          canonicalTargetCount: plannerPartitionReplay ? parsePacketEnvelope(prompt).targets.length : undefined,
          planning: planning ? {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (tool === "task") {
        ...
        if (
          ...
          result.reasons.push(`${task.type === "fresh-review" ? "operational-review" : "operational-explore"}-canonical-target-count-mismatch`)
        }
        const complete = result.reasons.length === 0
