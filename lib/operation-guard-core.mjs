...
import { CHILD_PLAN_SCHEMA, parseChildTerminal, planChildWork, semanticReviewAuthorityFromMessage, semanticReviewRequirement } from "./hybrid-workflow.mjs"
...
function hydrateSafetyState(safety, persisted) {
  if (!persisted || ![1, 2, 3, 4, 5].includes(persisted.version)) return
...
async function deterministicTaskPlan(args, state, directory, policy) {
  ...
  return planChildWork({
    ...
    role: type,
    authority: { repository: "local/workspace", head_sha: authorityHead, generation: state.editGeneration },
    semantic_review_authority: state.semanticReviewAuthority ?? "local-fresh-review",
    gate_class: type === "fresh-review" ? "semantic-review-evidence" : "focused-development",
...
export function createOperationGuard({ directory = process.cwd(), env = process.env, policy = policyFromEnv(env), stateDirectory, client } = {}) {
  ...
  return {
    ...
    "chat.message": async (input, output) => {
      ...
      if (declaration || semanticReviewChanged) await safetyStore.save()
    },
