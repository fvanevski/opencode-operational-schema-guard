...
function relativeCopyMoveTouchesWorkspace(command, workdir, workspaceRoot) {
  for (const raw of shellSegments(command)) {
    ...
    for (let index = 1; index < tokens.length; index += 1) {
      ...
      if (options && (token === "--target-directory" || token === "-t")) {
        ...
        if (target && isInsideDirectory(resolve(workdir, target), workspaceRoot)) return true
        index += 1
        continue
      }
      ...
      if (options && token.startsWith("-t") && token.length > 2) {
        const target = token.slice(2)
        if (target && isInsideDirectory(resolve(workdir, target), workspaceRoot)) return true
        continue
      }
      if (options && token.startsWith("-") && token !== "-") continue
      if (isInsideDirectory(resolve(workdir, token), workspaceRoot)) return true
    }
  }
  return false
}
...
function invocationOwnedByWorkspace(tool, args, directory) {
  ...
  if (relativeCopyMoveTouchesWorkspace(command, workdir, directory)) return true
