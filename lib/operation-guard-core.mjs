...
const FRESH_REVIEW_BOUNDED_SCOPE_SIGNAL = /\b(?:(?:current|this|provided|attached)\s+(?:pull request|PR|diff|patch|change(?:\s+set)?|changes)|(?:changed|modified)\s+(?:files?|paths?))\b/i
...
function freshReviewEnvelopeInferenceIsBounded(prompt, policy) {
  ...
  if (FRESH_REVIEW_OPEN_ENDED_SCOPE_SIGNAL.test(text) || FRESH_REVIEW_SCOPE_EXPANSION_SIGNAL.test(text) || FRESH_REVIEW_META_SCOPE_EXPANSION_SIGNAL.test(text) || FRESH_REVIEW_INVESTIGATIVE_UNBOUNDED_SIGNAL.test(text)) return false
  const targets = extractPaths(text, "/")
  if (targets.size > 0 && targets.size <= policy.taskExplicitTargets["fresh-review"]) return true
  return FRESH_REVIEW_BOUNDED_SCOPE_SIGNAL.test(text)
}
