...
function containsHeadProofAttempt(command) {
  const text = String(command)
  if (shellSegments(text).some((segment) => /^git\s+rev-parse\s+HEAD(?:\s|$)/i.test(canonicalGitSegment(segment)))) return true
  return shellNestedExecutionBodies(text).some((body) => containsHeadProofAttempt(body))
}
