...
const AUTHORITY_LABEL = /(?:^|[\r\n])[ \t]*(HEAD_SHA|EXPECTED_HEAD_SHA|EXPECTED_START_HEAD(?:_SHA)?|REQUIRED_START_HEAD_SHA|AUTHORITATIVE_HEAD_SHA|FINAL_HEAD_SHA|PR_HEAD_SHA|CANDIDATE_SHA|REQUIRED\s+STARTING\s+HEAD(?:\s+SHA)?|EXPECTED\s+STARTING\s+HEAD(?:\s+SHA)?|REQUIRED(?:\s+EXACT)?\s+HEAD|EXPECTED(?:\s+BRANCH)?\s+HEAD|REQUIRED\s+PR\s+HEAD)\s*(?:=|:)?\s*[`'"\s]*([0-9a-f]{40})\b/gi
