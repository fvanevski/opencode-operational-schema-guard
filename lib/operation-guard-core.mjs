...
function rootRelativeExplicitTarget(value, directory) {
  const candidate = String(value ?? "").replace(/[.!?;:]+$/g, "")
  if (!candidate || candidate === "." || candidate === ".." || candidate.includes("/") || candidate.includes("://") || candidate.startsWith("--")) return undefined
  const fileLike = ROOT_RELATIVE_FILE_LIKE_TARGET.test(candidate)
  let existing = false
  try {
    existing = lstatSync(resolve(directory, candidate)).isFile() || lstatSync(resolve(directory, candidate)).isDirectory()
  } catch {
    existing = false
  }
  return fileLike || existing ? candidate : undefined
}

function extractExplicitTargetPaths(text, directory) {
  ...
  for (const candidate of candidates) {
    const normalized = rootRelativeExplicitTarget(candidate, directory)
...
function partitionPacketFromOriginalPrompt(prompt, selectedPaths, directory) {
  ...
  for (const line of lines.slice(start, end)) {
    ...
    if (!match) return undefined
    const paths = [...extractExplicitTargetPaths(match[1], directory)].filter((path) => isInsideDirectory(path, directory))
...
async function deterministicTaskPlan(args, state, directory, policy) {
  ...
  for (const target of envelope.targets) {
    ...
    const paths = [...extractExplicitTargetPaths(target, directory)].filter((path) => isInsideDirectory(path, directory))
