...
For repository-owned mode the public gateway still owns the outer authority boundary. Before execution it fresh-fetches and proves exact base and head authority, inode-anchors the owner repository, proves the clean owner checkout is at the selected authority SHA, and creates a detached exact-authority control snapshot under `/tmp/opencode/control-worktrees/**`. `{repo_root}` is bound to that control snapshot rather than to the mutable owner checkout. The gateway opens immutable directory anchors for the native-result and canonical-evidence roots, opens and hashes a no-follow descriptor for the exact admitted runner, and proves the runner plus every declared control dependency against both its exact Git blob and current snapshot bytes. Optional SHA-256 pins may add a second content check.

Each repository-owned plan/run phase is Linux-supervised. The supervisor becomes a child subreaper before starting the descriptor-bound runner, so `setsid()`, double-fork, and daemonized descendants cannot escape the phase boundary; any survivor is killed/reaped and terminalizes the assessment. Landlock limits phase writes to the sanctioned `/tmp/opencode/verify` tree, the exact shared Git common directory required for fetch/worktree operations, and `/dev`, leaving the detached control snapshot unwritable by the runner and descendants. Inotify watches the runner, every declared integrity file, and their control-path ancestry, so external mutate-and-restore activity is detected even when final bytes are restored. Repository-owned mode therefore requires Linux with `/proc`, Python 3, Landlock, and inotify; inability to establish those primitives fails closed as infrastructure error rather than falling back to unsandboxed execution. After an optional plan and again immediately before run, the gateway revalidates admitted directory identities and runner/control inputs.
