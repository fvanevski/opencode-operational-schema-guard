...
- `runner.run_argv` to bind `{assessment_id}`, `{head_sha}`, `{pr_number}`, and `{workspace_root}`; and
- `runner.plan_argv`, when present, to bind `{head_sha}`, `{pr_number}`, and `{workspace_root}`.
...
The native runner writes inside its gateway-bound runtime:

```text
/tmp/opencode/verify/repository-owned/<assessment-id>/results/<assessment-id>/assessment.json
```

A repository-owned runner that manages worktrees/materials/results must use `{workspace_root}` (or an equivalent argv position carrying that placeholder) for its mutable lifecycle root. The gateway does not grant write authority to the shared `/tmp/opencode/verify` parent.
