...
For repository-owned mode the public gateway still owns the outer authority boundary. Before execution it fresh-fetches and proves exact base and head authority, inode-anchors the owner repository, proves the clean owner checkout is at the selected authority SHA, atomically reserves the control-snapshot destination, and creates a standalone shared-object, detached exact-authority control snapshot clone under `/tmp/opencode/control-worktrees/**`. A pre-existing destination blocks without mutation; failures before exact snapshot admission remove only the gateway-owned partial destination. The gateway opens and retains directory descriptors for both that control root and its disposable Git metadata; `{repo_root}` is rendered as the inherited `/proc/self/fd/<n>` control-root alias rather than a mutable pathname. Each supervisor phase `fchdir()`s to that descriptor and installs relative control watches through the same descriptor, so a pathname rename/substitution after validation cannot redirect runner imports or watch admission. The clone owns disposable Git metadata while reading existing immutable objects through Git's shared-object mechanism; the runner receives no write authority to the owner repository or its Git common directory. The gateway also opens immutable directory anchors for the native-result and canonical-evidence roots, opens and hashes a no-follow descriptor for the exact admitted runner, and proves the runner plus every declared control dependency against both its exact Git blob and current snapshot bytes. Optional SHA-256 pins may add a second content check.
