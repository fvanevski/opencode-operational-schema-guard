...
test("friction replay proves Central-led zero local Fresh-review/Verify and preserved local-Fresh-review control path", () => {
  ...
  assert.equal(central.after_metrics.actions_receipt_reuses, 2)
  assert.equal(central.after_metrics.actions_receipt_reuses, central.after_metrics.avoided_local_verify_launches)
  assert.equal(central.after_metrics.actions_receipt_reuses, central.after_metrics.avoided_full_suite_executions)
  assert.match(central.metric_provenance.after, /real planner lookup path/)
